...
description: "Write a CLAUDE.md for BetAway that enforces architecture rules, naming conventions, theme system usage, and code standards — turning your AI coding assistant into a senior team member."
...
In the previous two lessons, we set up the Xcode project and defined our MVVM architecture with SwiftData models, `@Observable` ViewModels, and a theme system. We have the skeleton. Now we need the brain — a file that tells our AI assistant exactly how to build inside that skeleton, every single time.
...
- Persistence: SwiftData — Views use @Query for reads,
  modelContext for writes
- ViewModels handle business logic and state, not persistence
- Swift 6 with strict concurrency
- iOS 17+, SwiftUI only, dark mode only
- Nunito custom font, 12 language support via LanguageManager
- Design system: ColorPalette, Typography, Spacing, AnimationPresets
...
Claude Code will propose something like: Project Overview, Architecture, Naming Conventions, File Organization, Theme System, Coding Standards, Testing Rules, and a Do NOT section.
...
**The architecture section is prescriptive, not descriptive.** It does not say "we use MVVM." It spells out how Views use `@Query` for reads and `@Environment(\.modelContext)` for writes, how ViewModels handle business logic, and how `AppRouter` manages navigation with a concrete `Tab` enum. Every prompt Claude Code processes will follow that pattern. If you ask for a new feature, the AI will define the model, add any enums, build the ViewModel, then create the View. Every time.

**The naming conventions include examples for every pattern.** There is no ambiguity about whether a view should be called `DashboardScreen`, `DashView`, or `DashboardView`. The rule says `[Feature]View`, the example says `DashboardView`. The same goes for ViewModels (`DashboardViewModel`) and models (`UserProfile`, `DailyCheckin`). Done.

**The Theme System section is a force multiplier.** By explicitly listing `ColorPalette`, `Typography`, `Spacing`, and `AnimationPresets` with concrete examples, you eliminate an entire class of inconsistency. Without it, the AI will hardcode `Color.white`, `Font.system(size: 17)`, and `Animation.spring(...)` everywhere. With it, every generated view uses `ColorPalette.textPrimary`, `Typography.body()`, and `AnimationPresets.smooth`. Consistency at scale.

**The Do NOT section is long — and that is intentional.** Fifteen explicit prohibitions. Each one prevents a specific class of mistake that AI models commonly make. "No ObservableObject" stops a pattern that appears in millions of training examples. "No AnyView" prevents a type erasure habit that kills SwiftUI performance. "No hardcoded colors" ensures the design system is actually used. These lines will save you more debugging time than any other part of the file.

### Before and After

Here is what the same prompt produces with a bad CLAUDE.md versus our production one.

**Prompt:** "Add a card showing the user's savings on the dashboard"

**With the vague CLAUDE.md** ("write clean code, use MVVM"):
```swift
// The AI might do this with hardcoded styles
...
Problems: Hardcoded font sizes instead of `Typography`. Hardcoded colors instead of `ColorPalette`. Hardcoded spacing instead of `Spacing`. Hardcoded corner radius instead of `Spacing.Radius`. Wrong naming convention. No design system usage at all.

**With our BetAway CLAUDE.md:**
```swift
// View — uses the full theme system
struct SavingsCardView: View {
    let profile: UserProfile

    var body: some View {
        VStack(spacing: Spacing.xs) {
            Text(LanguageManager.shared.localized("Money Saved"))
                .font(Typography.caption())
                .foregroundStyle(ColorPalette.textSecondary)
            Text(profile.totalSaved, format: .currency(code: AppConfig.defaultCurrency))
                .font(Typography.savingsCounter)
                .foregroundStyle(ColorPalette.success)
        }
        .padding(Spacing.md)
        .background(ColorPalette.surfacePrimary)
        .clipShape(RoundedRectangle(cornerRadius: Spacing.Radius.medium))
    }
}
```
...
- [ ] Architecture section specifies MVVM with `@Observable` ViewModels, `@Query` in Views, and `AppRouter` navigation
- [ ] Naming conventions cover Views, ViewModels, Models, and Managers with examples
- [ ] Theme System section covers ColorPalette, Typography, Spacing, and AnimationPresets
- [ ] Swift style rules include strict concurrency, guard-let, no force unwraps, enum constants via AppConfig
- [ ] File organization shows the complete folder structure (App, Core, Models, Navigation, ViewModels, Views)
- [ ] Testing section specifies Swift Testing and naming conventions
...
```
Create a MoodChartView that shows MoodRating values from DailyCheckin
entries over the last 7 days as a line chart.
```
...
Think about where chart data transformation should happen. It is the same answer as every other data question in this architecture — the ViewModel. The View should receive ready-to-render data points, not raw `DailyCheckin` or `UrgeLog` arrays. Your section should specify something like: "Chart ViewModels transform raw model data into chart-ready structs. Views never perform data aggregation."

Also consider: should the chart section specify `import Charts` only in Views, or is it acceptable in ViewModels too? Since Charts is a UI framework, it belongs in the View layer. And remember to specify that charts must use `ColorPalette` tokens for colors and `Typography` for any labels.
