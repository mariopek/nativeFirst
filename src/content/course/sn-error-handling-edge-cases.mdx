...
topics: ["Error Handling", "Typed Throws", "Loading State", "Empty State", "Graceful Degradation", "Retry Logic", "UX", "Moodbit"]
author: "Mario"
draft: false
pubDate: 2026-02-22
---

Your Moodbit app works. The happy path is solid — the user logs a mood, the AI generates an insight, the chart updates. Ship it, right?

Not yet. Open airplane mode and tap "Log Mood." What happens? Force quit the app mid-save and reopen it. What happens? Let the OpenAI API key expire. What happens?
...
- Implement graceful degradation so Moodbit falls back to on-device analysis when OpenAI is unreachable
...
Moodbit talks to an external AI service. That service will go down. The network will drop. The device will run out of storage. If you plan for these now, your app feels bulletproof. If you do not, it feels fragile.
...
```
I need to add production-quality error handling to Moodbit.
...
```
Implement the error handling system for Moodbit based on our plan.
...
enum AppError: Error, Sendable, LocalizedError {
    ...
    var userMessage: String {
        ...
        case .network(.noConnection):
            "No internet connection. Your mood will be saved locally and synced when you are back online."
        ...
        case .persistence(.saveFailed):
            "Could not save your mood entry. Please free up some storage and try again."
        case .persistence(.fetchFailed):
            "Could not load your mood history. Pull down to retry."
        case .persistence(.storageFull):
            "Your device storage is full. Free up space to continue logging moods."
        case .persistence(.migrationFailed):
            "There was a problem updating the app data. Please restart Moodbit."
        case .health(.permissionDenied):
            "Moodbit needs Health access to correlate mood with activity. Open Settings to grant permission."
...
struct FallbackAnalyzer: Sendable {
    ...
    func generateFallbackInsight(moodScore: Int, note: String?) -> String {
        guard let note, !note.isEmpty else {
            return moodLevelInsight(for: moodScore)
        }

        let sentiment = analyzeSentiment(for: note)

        if sentiment > 0.3 && moodScore >= 7 {
            return "Your note reflects a positive mindset. Keep building on what is working well today."
        } else if sentiment < -0.3 && moodScore <= 4 {
    ...
    private func moodLevelInsight(for score: Int) -> String {
        switch score {
        case 1...3:
            "Low mood is a signal, not a verdict. What is one small thing that usually helps you feel a bit better?"
        case 4...6:
            "A neutral mood is perfectly normal. Notice what is present in your day — sometimes calm is underrated."
        case 7...10:
            "Great energy today. Take a moment to appreciate what is contributing to this feeling."
        default:
            "Mood logged. Check back later for insights on your patterns."
        }
    }
}
```
...
func withRetry<T: Sendable>(
    ...
    let logger = Logger(subsystem: "com.moodbit", category: "Retry")
...
```
Now update MoodEntryViewModel to use the error handling system.

When the user logs a mood:
...
3. Save the mood entry with whatever insight we got
4. If saving fails, THEN show the error

For loading the mood history:
1. Use LoadingState<[MoodEntry]> — handle all four states
...
@Observable
final class MoodEntryViewModel: @unchecked Sendable {
    var historyState: LoadingState<[MoodEntry]> = .idle
    var isSaving = false
    ...
    func saveMood(score: Int, note: String?, emoji: String) async {
        ...
        do {
            ...
            logger.info("Mood entry saved. Insight source: \(insight.source.rawValue)")
            await loadHistory()
        } catch {
...
Notice the degradation strategy: the user taps "Save," the app tries OpenAI with retries, and if all attempts fail, it silently falls back to `NLTagger`. The user still gets an insight. They still get their mood saved. They never see an error for the AI failure because it is not their problem to solve.
...
1. **Airplane mode** — Log a mood. The app should fall back to on-device analysis, save successfully, and show no error
2. **Invalid API key** — The retry utility should bail immediately (non-transient), fall back to on-device analysis
3. **Empty state** — Delete all mood entries. The EmptyStateView should appear with a prompt to log the first mood
4. **Storage full** (simulate by throwing in the repository) — The user should see a specific "free up space" message, not "Something went wrong"
...
#Preview("No Entries") {
    EmptyStateView(
        ...
        description: "Log your first mood to start tracking patterns.",
...
- [ ] Turning off Wi-Fi and logging a mood produces an on-device insight silently — no error shown to the user
...
- [ ] `EmptyStateView` appears when the mood history is empty

## Challenge

**Add offline queueing:** When a mood is saved with an on-device fallback insight, queue it for AI re-analysis. When the network comes back (use `NWPathMonitor`), automatically re-analyze queued entries with OpenAI and update the insights in the background. The user should never need to do anything — the upgrade from on-device to AI insight happens silently.

*Hint:* Add an `InsightSource` field to `MoodEntry`. Query for entries where `insightSource == .onDevice`. Use a background `Task` that monitors `NWPathMonitor` and triggers re-analysis when the path becomes `.satisfied`.
