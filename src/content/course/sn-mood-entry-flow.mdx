...
description: "Build BetFree's dual check-in system — Morning Pledge and Evening Check-in — plus a full urge resistance flow with a 15-minute breathing timer, coping toolkit, and structured logging using SwiftData."
...
The dashboard shows existing check-ins. The design system makes them look consistent. Now we build the screens where users actually check in and resist urges — the two most important interactions in BetFree.

BetFree has a dual check-in system. In the morning, users take a **pledge** — a commitment to stay gambling-free for the day, paired with a mood rating and a reason. In the evening, they complete a **check-in** — reflecting on whether they stayed gambling-free, their mood, any triggers they encountered, and optional notes. Beyond check-ins, BetFree has a full **urge resistance flow** — when a user feels the pull to gamble, they can start a 15-minute timer with guided breathing, access a distraction toolkit (breathing exercises, grounding, movement, emergency contacts), and log the urge with intensity, trigger, and coping strategy.

This is not a simple form. A recovery check-in is a vulnerable moment. The user is pausing their day to reflect on their progress and urges. The UX needs to respect that. It should feel smooth, guided, and supportive — not like filling out a survey.
...
- Creating the `PledgeViewModel` — an `@Observable` class managing both pledge and check-in submission with SwiftData persistence
...
- Wiring `FlowLayout` — a custom `Layout` that wraps chips to new lines — for trigger and coping strategy selection
...
BetFree's approach solves this with two focused moments per day — morning and evening — each with a distinct purpose. The morning pledge sets intention. The evening check-in builds accountability. The urge flow provides real-time support during the hardest moments. Together they create a rhythm that keeps users engaged without feeling like homework.

This dual check-in pattern applies beyond recovery tracking. Any app that benefits from repeated daily engagement — habit trackers, mood journals, fitness apps — can use a time-gated, purpose-specific check-in model to boost retention.
...
```
I need to build the check-in system for BetFree — a gambling
recovery app. Before writing code, help me plan the architecture.

The app has TWO check-in types:
1. Morning Pledge — user commits to stay gambling-free today,
...
2. Evening Check-in — user reports whether they stayed
...
1. Acknowledge phase — reassuring message, option to start timer
2. Timer phase — 15-minute countdown with breathing animation,
   progress bar, encouragement text that changes over time,
   distraction toolkit (breathing, grounding, movement, call)
3. Log phase — intensity slider (1-10), trigger selection,
   coping strategy selection, did-you-resist toggle, notes
4. Complete phase — celebration with stats
...
**Models:**
- **Enums.swift** — `MoodRating`, `CheckinType`, `CopingStrategy`, `PredefinedTrigger`, `RiskLevel`
- **DailyCheckin.swift** — SwiftData `@Model` for both morning pledges and evening check-ins
- **UrgeLog.swift** — SwiftData `@Model` for urge resistance entries

**ViewModels:**
- **PledgeViewModel.swift** — form state for both morning and evening flows, submission logic
- **UrgeViewModel.swift** — phase management, timer, breathing animation, submission logic

**Views:**
- **PledgeContainerView.swift** — routes between morning pledge, evening check-in, or "all done"
- **MorningPledgeView.swift** — the morning pledge form
- **EveningCheckinView.swift** — the evening check-in form
- **DashboardPledgeCard.swift** — the dashboard card that opens the pledge flow
- **UrgeContainerView.swift** — routes between urge phases
- **UrgeAcknowledgeView.swift**, **UrgeTimerView.swift**, **UrgeLogView.swift**, **UrgeCompleteView.swift** — one view per urge phase
- **BreathingExerciseView.swift**, **GroundingExerciseView.swift**, **MovementSuggestionsView.swift** — distraction toolkit sheets
...
```swift
// What AI generates without guidance — do NOT do this
...
```swift
// Enums.swift — domain vocabulary for BetFree
...
- `MoodRating` is an `Int`-backed enum (1-5) with both an `emoji` and a `label`. The raw value maps directly to the SwiftData `mood: Int` field — no conversion needed.
...
- `CopingStrategy` mirrors the distraction toolkit in the urge flow — breathing, grounding, calling someone, physical activity, journaling, distraction.

Now the SwiftData models.

```swift
// DailyCheckin.swift — SwiftData model for both pledge and check-in
...
**Key design decision:** Both models store enums as raw `String` values (e.g., `checkinTypeRaw`, `triggerRaw`). SwiftData cannot persist Swift enums directly, so we use raw-value storage with computed property wrappers that convert back and forth. The `triggersRaw: [String]` array stores multiple triggers — SwiftData handles `[String]` natively.

The `DailyCheckin` model serves both morning pledges and evening check-ins. The `checkinType` field distinguishes them. This is intentional — it lets `UserProfile` maintain a single `checkins` relationship and makes queries simpler ("give me all check-ins this week" works without joining two tables).
...
- `@Observable` with `final class` — correct modern pattern, not `ObservableObject`
...
- **`submitPledge` vs `submitCheckin`** — two separate methods, not a single `save()` with conditionals. Each creates a `DailyCheckin` with the appropriate `checkinType`. This is explicit and easy to trace.
- **Haptic feedback is conditional.** `submitPledge` always fires `HapticManager.success()`. `submitCheckin` only fires it when `gamblingFree` is true. If the user relapsed, we do not celebrate — we respect the moment. This is a UX detail that matters deeply in a recovery app.
- **`TimeOfDay`** — the ViewModel knows what time window we are in. The container uses `hasPledgedToday()` / `hasCheckedInToday()` on the profile instead, which is more reliable (it checks actual data, not just the clock).
- **`pledgeReasons` is a static property** — localized strings for the quick-select chips. These are not user-editable; they are curated reasons that resonate with gambling recovery.

### PledgeContainerView — The Router
...
- The routing logic is a simple if/else chain: both done → `AllDoneView`, pledge not taken → `MorningPledgeView`, pledge taken → `EveningCheckinView`. No enum-based state machine needed — the `UserProfile` is the source of truth.
...
- `@State private var viewModel = PledgeViewModel()` — the ViewModel is created fresh each time the sheet presents. When the sheet dismisses and re-presents, state is clean.
- The `navigationTitle` updates dynamically based on what the user needs to do. The user sees "Morning Pledge" or "Evening Check-in" — never a generic "Check-in."

### MorningPledgeView — The Pledge
...
- **`@Namespace private var moodNamespace`** — used by `MoodButton` for `matchedGeometryEffect`. When the user taps a different mood, the highlight background animates smoothly from one emoji to the next instead of popping.
- **`staggeredAppear(index:isVisible:)`** — a custom view modifier that delays each section's entrance animation by its index. The header appears first, then the pledge text, then mood, then reasons, then the button. This creates a cascading reveal that feels intentional.
...
- **`FlowLayout`** — a custom `Layout` that wraps chips to the next line when they exceed the available width. This is essential because the pledge reasons have variable lengths and a `LazyVGrid` would create ugly gaps.

Here is the `MoodButton` component that makes the mood selector feel physical:

```swift
// MoodButton — used in MorningPledgeView
...
- **The relapse section appears conditionally** — `if !viewModel.gamblingFree { relapseSection }` with `.transition(.opacity.combined(with: .move(edge: .top)))`. When the user taps "No," the section slides in from above with a fade. It contains an empathetic message ("Relapse is part of recovery"), an amount field, and a helpline number. This is not judgmental — it is supportive.
...
### UrgeViewModel — Timer, Breathing, and Phase Management
...
struct UrgeAcknowledgeView: View {
    ...
    var body: some View {
        VStack(spacing: Spacing.xxl) {
            ...
            BFButton(title: "I can handle this — skip timer", style: .ghost) {
                viewModel.moveToLog()
            }
            .padding(.horizontal, Spacing.xl)
            .staggeredAppear(index: 5, isVisible: isVisible)
...
The timer phase is the most visually rich — a breathing circle that expands and contracts, a countdown timer with `.contentTransition(.numericText())`, a progress bar, time-dependent encouragement text, and the distraction toolkit:
    ...
    var body: some View {
        VStack(spacing: Spacing.xl) {
            ...
            Button {
                ...
                Text("I'm feeling better — log this urge")
                    .font(Typography.subheadline(weight: .medium))
                    .foregroundStyle(ColorPalette.textTertiary)
            }
...
The distraction toolkit buttons open sheets for each activity — `BreathingExerciseView` (4-7-8 breathing technique with animated circle), `GroundingExerciseView` (5-4-3-2-1 senses exercise), `MovementSuggestionsView` (walk, stretch, push-ups, dance, clean), and `EmergencyContactsSheetView`. Each sheet can be dismissed independently — the timer keeps running underneath.

### UrgeLogView — Structured Logging
...
The `intensityColor` computed property changes the slider tint based on the value — green for mild (1-3), yellow for moderate (4-6), red for severe (7-10). This is immediate visual feedback that communicates severity without the user reading a number.

The `TriggerChip` and `StrategyChip` components follow the same pattern — icon + text in a capsule, with toggle selection:

```swift
// TriggerChip — used in UrgeLogView
...
### DashboardPledgeCard — The Entry Point
...
The card is a `Button` that opens the pledge flow as a `.sheet`. The CTA text and icon change based on what the user needs to do next — "Take Today's Pledge" in the morning, "Complete Evening Check-in" in the evening. When both are done, the button is disabled and a "Complete" label appears in the header.
...
Every method checks `isEnabled` first — if the user has disabled haptics in settings, all methods become no-ops. The `badgeUnlocked()` method is a compound effect: heavy impact followed by success notification after a 150ms delay. This creates a satisfying "thunk + chime" feel.

## Verify Phase

Build the project. Run it in the simulator and verify each flow:

1. **Morning Pledge.** Open the dashboard card. The morning pledge should appear. Select a mood — the `matchedGeometryEffect` should smoothly move the highlight. Pick a reason chip. Tap "Take My Pledge." You should feel a success haptic, see the toast ("Pledge taken!"), and the sheet should auto-dismiss after 2.5 seconds.

2. **Evening Check-in.** Open the card again. The evening check-in should now appear. The "Did you stay gambling-free?" question should show "Yes!" selected by default. Tap "No" — the relapse section should slide in with the amount field and helpline info. Tap "Yes!" again — it should slide back out. Select a mood, write optional notes, and submit. If gambling-free, you should see the celebration toast.
...
4. **Urge flow — full path.** Open the urge flow. The acknowledge screen should show a pulsing circle with "It's okay." Tap "Start Urge Timer." The breathing circle should expand and contract on a 4-second cycle. The timer should count down from 15:00. The encouragement text should change as time passes. Tap a distraction toolkit button — the sheet should open while the timer continues. Skip the timer. The log screen should show your elapsed time. Select an intensity, trigger, strategy, and tap "Save." The completion screen should show a celebration.

5. **Edge cases.** Dismiss the urge flow mid-timer — the timer Task should cancel cleanly. Start the urge flow again — state should be fresh. Log an urge where you did not resist — no celebration haptic, but an empathetic message.

## Checkpoint

Let us confirm what we built and why each piece matters.

**PledgeViewModel** manages both morning and evening flows in a single `@Observable` class. It has separate `submitPledge` and `submitCheckin` methods that create `DailyCheckin` records with the appropriate `CheckinType`. Haptic feedback is conditional — success for gambling-free days, silence for relapses.

**PledgeContainerView** routes between states using `hasPledgedToday()` and `hasCheckedInToday()` on the `UserProfile`. This is data-driven routing, not time-based — if the user pledges at 11pm, the morning pledge still works.
...
1. `EveningCheckinView` — add the trigger selection section inside the `if !viewModel.gamblingFree` block

Prompt Claude Code for the change. Review the diff. Verify that selected triggers are saved to the `DailyCheckin` model. This is the iterative workflow in action — one focused change at a time, not one giant prompt for everything.
