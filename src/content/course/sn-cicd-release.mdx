---
title: "CI/CD & Release"
description: "Set up automated builds with GitHub Actions, distribute through TestFlight, implement semantic versioning, and establish the post-launch monitoring workflow."
...
topics: ["CI/CD", "GitHub Actions", "TestFlight", "Semantic Versioning", "Automation", "Xcode Cloud", "Release", "Moodbit"]
author: "Mario"
draft: false
pubDate: 2026-02-22
---

You can build the app on your machine and upload it through Xcode. That works exactly once. The second time, you forget to increment the build number. The third time, you push a commit that breaks the tests but upload anyway because "it works on my machine." By the fifth time, you are dreading releases.

This lesson sets up the pipeline that removes you as the bottleneck. Push to `main`, and the machine takes over: build, test, archive, upload to TestFlight. Every time, the same way, with no human error.
...
```
I need to set up CI/CD for an iOS app called "Moodbit."
...
```
Generate a version management setup for Moodbit:
...
2. Explain the semantic versioning rules for our app:
   - MAJOR: breaking changes (data model migration, dropped iOS version)
   - MINOR: new features (new screen, new AI capability)
...
for Moodbit with these requirements:
...
1. "test" — runs on every push and PR
   ...
   - Use xcodebuild with -scheme Moodbit -destination
...
enum AppVersion {
    ...
    static func logAtLaunch() {
        logger.info("Moodbit \(Self.display) launched.")
    }
}
```
...
env:
  ...
  ARCHIVE_PATH: build/Moodbit.xcarchive
        ...
        run: |
          if [ -n "$SLACK_WEBHOOK_URL" ]; then
            curl -s -X POST "$SLACK_WEBHOOK_URL" \
              ...
              -d "{\"text\":\"Moodbit build ${{ github.run_number }} uploaded to TestFlight.\"}"
          fi
      ...
      - name: Notify Slack on failure
        ...
        run: |
          if [ -n "$SLACK_WEBHOOK_URL" ]; then
            curl -s -X POST "$SLACK_WEBHOOK_URL" \
              -H 'Content-Type: application/json' \
              -d "{\"text\":\"Moodbit build ${{ github.run_number }} FAILED. Check: ${{ github.server_url }}/${{ github.repository }}/actions/runs/${{ github.run_id }}\"}"
          fi
```
...
Next up: **Bonus Lesson B.1 — CloudKit Sync**, where we add multi-device sync to Moodbit so users never lose their mood history.
