...
description: "Optimize BetAway for production — lazy loading, SwiftData query tuning, image caching, launch time reduction, and background processing patterns that keep your app fast."
...
difficulty: "advanced"
topics: ["Performance", "LazyVStack", "SwiftData Optimization", "Image Caching", "Launch Time", "Background Processing", "Pagination", "BetAway"]
author: "Mario"
draft: false
pubDate: 2026-02-22
---

BetAway works. The error handling is solid. The accessibility is polished. A user with twelve entries opens the app and everything feels instant. That same user logs check-ins for a year, accumulates 800 entries, opens the app on a cold morning, and the launch takes three seconds. They scroll through their recovery history and every swipe hitches. They tap the Insights tab and the chart takes a full second to appear.

They do not file a bug. They do not leave a review. They delete your app and try another one.

Performance is the feature nobody sees until it is missing. In this lesson, we measure what is actually slow, fix it with targeted optimizations, and verify that every change made things faster — not just different. No guessing. No premature optimization. Instruments first, code changes second.

## What You'll Learn

- Replace `VStack` with `LazyVStack` and paginate recovery history with `FetchDescriptor` so only 50 entries load at a time instead of the entire database
...
- Build an image cache for check-in photos so the same thumbnail never decodes from disk twice during a scroll session
...
Beyond Apple's threshold: every 100ms of delay reduces user engagement measurably. BetAway talks to an external AI service, reads from a local database, renders charts, and — if users attach photos — decodes images. Each of those is a potential bottleneck. Let us find out which ones actually are, and fix only what matters.
...
```
I need to optimize BetAway's performance before shipping.
Do NOT write code yet — create a profiling and optimization plan.

Current architecture:
- SwiftData for persistence (CheckIn model with score, note,
  emoji, insight, date, insightSource, optional photoData)
- OpenAI API for insight generation (network call per entry)
- Charts framework for recovery history visualization
...
2. The recovery history list might be using VStack instead of LazyVStack
...
The AI should produce an ordered list of measurements before optimizations. That order is critical — you need a baseline before changing anything.

**Keep** — "Measure launch time with Instruments Time Profiler before changing anything." Correct. Without a baseline number, you cannot prove an optimization helped. You are just guessing.

**Keep** — "Profile scroll performance at 1000+ entries with test data." The AI should suggest generating test data first, then profiling. Performance testing with 5 entries tells you nothing about production behavior.

**Keep** — "Use os_signpost markers for each critical path." Signposts show up in Instruments as named intervals, so you can see exactly how long each operation takes. This is how you get hard numbers, not estimates.

**Question** — If the AI jumps straight to "replace VStack with LazyVStack" without suggesting measurement first, push back. That is probably the right fix, but confirm with data. Premature optimization creates complexity without evidence it was needed.
...
// They add a manual cache for recovery check-ins:
private var cachedEntries: [CheckIn]?
...
recovery check-ins for performance testing.

- Generate entries spanning 365 days
- 1-3 entries per day (random)
- Random recovery scores weighted toward 5-8 (realistic distribution)
...
```
Optimize BetAway for performance at scale (1000+ entries).
Apply these optimizations in order:

1. LAZY LOADING: Replace VStack with LazyVStack in
   RecoveryHistoryView. Use FetchDescriptor with fetchLimit and
   fetchOffset for true database-level pagination — 50 entries
...
3. IMAGE CACHING: Build an ImageCache actor that decodes
   ...
   recovery history row should check the cache before decoding.

4. BACKGROUND AI PROCESSING: Move the OpenAI API call off
   the main thread. Save the check-in immediately with a
...
**Optimization 1 — LazyVStack with True Pagination:**

The before and after tells the whole story.

```swift
// BEFORE — loads ALL entries into memory, renders ALL rows at once
        ...
        ScrollView {
            VStack(spacing: 0) {                 // Every row created
                ForEach(entries) { entry in       // All 1000 entries
                    CheckInRow(entry: entry)    // in memory
                }
            }
        }
    }
...
The `@Query` fetches every `CheckIn` from the database the moment this view appears. `VStack` creates all 1000 row views immediately. On a large dataset, this is an 800ms stall before the first frame renders.

```swift
// AFTER — fetches 50 at a time, only visible rows exist
import SwiftUI
import SwiftData
import os
...
final class RecoveryHistoryViewModel {
    ...
    private let signposter = OSSignposter(
        subsystem: "com.nativefirst.betaway", category: "Performance"
    )
    ...
    private func fetchPage(offset: Int) -> [CheckIn] {
...
- ✅ `FetchDescriptor` with `fetchLimit` and `fetchOffset` — the database returns only 50 rows per page, not all 1000
- ✅ `os_signpost` on each page load — measurable in Instruments Time Profiler
- ✅ `hasMore` flag prevents unnecessary queries once all data is loaded
- ✅ `isLoadingPage` guard prevents duplicate fetches from rapid scroll events
- ⚠️ The ViewModel holds `ModelContext`, which is not `Sendable`. This is fine as long as the ViewModel stays on the main actor. If the AI marks it `@Sendable`, question it — `ModelContext` must be used on a single thread
...
**Optimization 2 — SwiftData Query Tuning:**

Two fixes here. First, the Insights chart should only fetch the last 30 days:

```swift
// BEFORE — fetches ALL entries, filters in Swift
struct InsightsView: View {
    @Query(sort: \CheckIn.date)
    private var allEntries: [CheckIn]
    ...
    var body: some View {
        ...
        Chart(recent) { entry in
            LineMark(
                ...
                y: .value("Recovery Score", entry.score)
            )
        }
    }
}
...
```swift
// AFTER — filter at the database level with #Predicate
struct InsightsView: View {
    @Query(
        filter: #Predicate<CheckIn> {
            $0.date > Date.now.addingTimeInterval(-30 * 24 * 60 * 60)
        },
        sort: \CheckIn.date
    )
    private var recentEntries: [CheckIn]
                ...
                VStack(spacing: 20) {
                    if recentEntries.isEmpty {
                        ContentUnavailableView(
                            ...
                            description: Text("Log check-ins for a few days to see trends.")
                        )
                    } else {
                        recoveryChart
                        statsSection
...
- ✅ `#Predicate` filters at the database level — SQLite returns only matching rows, not all 1000
- ✅ The chart renders at most ~90 data points (3/day for 30 days), which the Charts framework handles without issue
- ❌ If the AI filters in Swift after a full fetch, reject it. That defeats the purpose of a database query

Second, for the home screen where we just need a count:

```swift
// BEFORE — loads all entries just to count them
@Query private var entries: [CheckIn]

var body: some View {
    Text("\(entries.count) check-ins logged")
    // This fetched every CheckIn into memory just for .count
}

// AFTER — fetch only the count
@Observable
...
- ✅ `fetchCount()` executes a `SELECT COUNT(*)` — no objects materialized, no memory allocated for rows
- ✅ On 1000 entries, the difference is negligible in time but significant in memory: 0 bytes vs ~12 MB of hydrated model objects

**Optimization 3 — Image Cache for Check-In Photos:**
...
actor ImageCache {
    ...
    private let signposter = OSSignposter(
        subsystem: "com.nativefirst.betaway", category: "ImageCache"
    )
...
- ✅ `actor` provides thread-safe access without manual locking — Swift concurrency handles synchronization
- ✅ `CGImageSourceCreateThumbnailAtIndex` decodes directly to the target size — never decodes the full-resolution image into memory
- ✅ `NSCache` automatically evicts under memory pressure — no manual cleanup needed
- ✅ In-flight deduplication prevents decoding the same image twice when rapid scroll events fire multiple requests
- ✅ `os_signpost` on each decode — visible in Instruments to verify cache hits vs misses

Using it in a check-in row:
...
- ✅ `.task(id: entry.id)` — cancels and restarts if the row is reused for a different entry during scroll
- ✅ The thumbnail is loaded asynchronously — no main thread blocking
- ✅ On subsequent appearances (scrolling back), the cache returns instantly

**Optimization 4 — Background AI Processing:**
...
@Observable
final class CheckInLogViewModel {
    var isSaving = false
    var savedEntry: CheckIn?
    ...
    func saveCheckIn(recoveryScore: Int, note: String?, emoji: String) async {
        isSaving = true
        let saveState = signposter.beginInterval("SaveCheckIn")

        // Save immediately with placeholder — user sees instant feedback
        let entry = CheckIn(
            score: score,
            note: note,
            emoji: emoji,
        ...
        do {
            ...
            savedEntry = entry
            isSaving = false
            signposter.endInterval("SaveCheckIn", saveState)
        } catch {
            isSaving = false
            signposter.endInterval("SaveCheckIn", saveState)
            return
        ...
        // Generate AI insight in the background — UI is already updated
        pendingInsight = true
        ...
        do {
            ...
            // Insight update failed — entry still saved with placeholder.
            // Not critical. The user's check-in is persisted.
        }
...
- ✅ The check-in saves in under 100ms — the user sees confirmation immediately
- ✅ The AI call runs asynchronously after the save — no UI freeze, no spinner
- ✅ The entry updates in place when the insight arrives — "Generating your insight..." transitions to the real insight
- ✅ Signposts separate save time from insight generation time in Instruments — you can see exactly where time goes
- ⚠️ If the AI puts the entire save + insight generation into one `await` chain that blocks the UI until the insight returns, reject it. The save must complete independently

**Optimization 5 — Launch Time:**
...
struct BetAwayApp: App {
    private let signposter = OSSignposter(
        subsystem: "com.nativefirst.betaway", category: "Launch"
    )
...
```swift
// BAD — all ViewModels created at launch regardless of tab
struct ContentView: View {
    @State private var historyVM = RecoveryHistoryViewModel(...)   // Created at launch
    ...
    var body: some View {
        TabView {
            CheckInLogView()
            RecoveryHistoryView(viewModel: historyVM)       // Tab is lazy...
...
struct RecoveryHistoryView: View {
    ...
    var body: some View {
        ...
        .task {
            if viewModel == nil {
                let vm = RecoveryHistoryViewModel(context: context)
                vm.loadInitialPage()
                viewModel = vm
            }
        }
...
- ✅ The ViewModel is `nil` until `.task` fires — which only happens when the tab is selected
- ✅ Data fetching happens after the view appears, not during `init()`
- ✅ The `ProgressView` shows for a single frame at most — the initial page loads in ~45ms
...
1. Prefetch the next page when the user scrolls within 10
   entries of the page boundary — eliminate visible loading.
...
```swift
// Add to RecoveryHistoryViewModel
func prefetchIfNeeded(currentEntry: CheckIn) {
    guard hasMore, !isLoadingPage else { return }
...
- ✅ When the user scrolls past entry 40 of a 50-entry page, the next page starts loading
- ✅ By the time they reach entry 50, the next 50 are already appended
- ✅ The `ProgressView` at the bottom is rarely visible — data arrives before the user scrolls there
...
1. **Launch time** — Use the Time Profiler instrument. Measure from `main()` to first frame. Your signpost `AppLaunchToFirstFrame` should show under 1 second for a warm launch. On a cold launch, target under 2 seconds. If it exceeds this, check if any `init()` methods are doing work they should defer.

2. **Scroll performance** — Open recovery history and scroll quickly through all entries. Use the SwiftUI instrument (available in Xcode 16+). No `body` evaluation should take longer than 16ms — that is one frame at 60fps. If you see spikes, check if image decoding is happening on the main thread.

3. **Memory** — Open the Allocations instrument. Navigate to recovery history, scroll through all entries, navigate back. Memory should return close to baseline. If it grows linearly with scroll distance, you have a retain cycle or unbounded cache. The `NSCache` in `ImageCache` should handle eviction automatically.

4. **Page loading signposts** — In Instruments, filter by the "Performance" category. `LoadInitialPage` should be under 50ms. `LoadNextPage` should be under 30ms. `DecodeThumbnail` should show cache hits (near 0ms) on second scroll and cache misses (~5-15ms) on first scroll.

5. **AI processing** — Log a check-in and watch the Time Profiler. The `SaveCheckIn` signpost should complete in under 100ms. The `GenerateInsight` signpost will be 1-3 seconds but must run on a background thread — verify it does not appear on the main thread track.
...
Here is the complete `RecoveryHistoryView` with all optimizations applied — lazy loading, pagination, prefetching, image caching, and deferred ViewModel initialization:
...
struct RecoveryHistoryView: View {
    ...
    private func historyContent(
        _ viewModel: RecoveryHistoryViewModel
    ) -> some View {
        Group {
            if viewModel.entries.isEmpty {
                ContentUnavailableView(
                    ...
                    description: Text("Log your first check-in to start tracking patterns.")
                )
            } else {
                ScrollView {
                    LazyVStack(spacing: 0) {
...
| Background AI processing | UI blocks 1-3s during insight generation | Save completes under 100ms | Time Profiler, signposts |
...
3. Memory does not grow unbounded when scrolling through all entries and back — check with Allocations instrument
4. Logging a check-in shows instant feedback. The save completes before the AI insight arrives
...
7. No `VStack` inside `ScrollView` with `ForEach` remains anywhere in the codebase — search the project to confirm
