...
difficulty: "advanced"
topics: ["CloudKit", "SwiftData Sync", "iCloud", "Conflict Resolution", "Multi-Device", "Moodbit"]
author: "Mario"
draft: false
pubDate: 2026-02-22
---
...
- ✅ **Container identifier** — an iCloud container identifier like `iCloud.com.yourname.moodbit`, created in the Apple Developer portal or automatically by Xcode when you check the CloudKit capability.
...
/// Centralizes CloudKit and ModelContainer configuration for Moodbit.
enum CloudKitConfiguration {
    ...
    static let containerIdentifier = "iCloud.com.yourname.moodbit"
...
**Why last-write-wins?** SwiftData with CloudKit does not offer built-in field-level merge. The entire record is replaced by whichever device wrote last (by timestamp). For a mood journal, this is acceptable — users rarely edit the same entry on two devices simultaneously. For collaborative apps (shared documents, team task lists), you would need a custom merge strategy or a different sync backend entirely.
...
4. `MoodEntry` has no `@Attribute(.unique)` constraints on any property
5. All non-optional properties in `MoodEntry` have default values
6. App builds and runs with zero warnings under Swift 6 strict concurrency
7. CloudKit Dashboard shows the `MoodEntry` record type after first launch
...
if !syncMonitor.isCloudAvailable {
    ContentUnavailableView(
        ...
        description: Text("Sign into iCloud in Settings to sync your mood entries across devices.")
    )
}
```
</details>
...
Next up: **Bonus Lesson 9.2 — Share Extension & Beyond**, where we build a Share Extension for quick mood logging from any app and chart your path forward as an AI-augmented iOS developer.
