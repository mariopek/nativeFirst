...
description: "Master a systematic debugging workflow combining Xcode instruments, LLDB, os.Logger, and AI-assisted root cause analysis for Moodbit"
...
topics: ["Debugging", "os.Logger", "LLDB", "Instruments", "OSSignposter", "SwiftUI", "Performance", "Moodbit"]
author: "Mario"
draft: false
pubDate: 2026-02-22
order: 23
...
This lesson builds that toolkit for Moodbit.
...
```
I need to build a debugging and diagnostics toolkit for Moodbit,
a SwiftUI mood tracking app. Before writing code, help me plan
the approach.

The app has:
- SwiftData persistence layer (MoodRepository)
...
- ✅ **Single subsystem, multiple categories.** The subsystem should be your bundle identifier (e.g., `com.yourname.Moodbit`). Categories map to functional areas: `persistence`, `viewModel`, `network`, `ai`, `sync`. This structure lets you filter all Moodbit logs with one subsystem filter, then drill into specific categories.
...
```
Create the debugging toolkit for Moodbit. Generate these files:

1. Logger+Extensions.swift
   - Define a Logger enum or struct with static loggers for
     each Moodbit subsystem category: persistence, viewModel,
     network, ai, sync, ui
...
- ⚠️ **Watch for string interpolation privacy.** `os.Logger` redacts dynamic strings by default in release builds. The AI may forget to add `privacy: .public` where appropriate (e.g., mood level names are not sensitive). Conversely, it should use `.private` for user-entered notes.
...
**6. LLDB commands work in a live session.** Set a breakpoint in `MoodTimelineViewModel.loadEntries()`. When it hits, try these commands in the Xcode debug console:

```
(lldb) po self.entries.count
(lldb) v self.isLoading
(lldb) expr self.isLoading = true
(lldb) breakpoint set -n "MoodRepository.fetchAll"
```
...
### Moodbit/Debug/Logger+Extensions.swift

```swift
import os
import Foundation

// MARK: - Moodbit Logger Namespace

enum Log {
    /// Subsystem identifier for all Moodbit logging.
    private static let subsystem = Bundle.main.bundleIdentifier ?? "com.moodbit.app"
...
//
// Keep this reference handy when debugging Moodbit in Xcode.
//
// ┌─────────────────────────────────────────────────────────────────┐
// │ Command          │ Description                                  │
// ├─────────────────────────────────────────────────────────────────┤
...
### Moodbit/Debug/DiagnosticReport.swift
...
struct DiagnosticReport: Sendable {
    ...
    func formattedReport() -> String {
        ...
        lines.append("MOODBIT DIAGNOSTIC REPORT")
...
### Moodbit/Debug/DebugOverlayView.swift
...
Use these commands in the Xcode debug console when a breakpoint hits. This table covers the commands you will use most often when debugging Moodbit.
...
| `br s -n <func>` | Set breakpoint on function name | `br s -n "loadEntries"` |
...
Follow this checklist when profiling Moodbit with Instruments.
...
ALLOCATIONS (finding memory leaks):
  ...
  [ ] Filter by "Moodbit" to see only your allocations
...
- [ ] `Logger+Extensions.swift` compiles with `import os` and defines loggers for all Moodbit subsystems
...
- [ ] LLDB `po`, `v`, and `expr` commands work at a breakpoint in `MoodTimelineViewModel`
...
```swift
func generateInsight(for entries: [MoodEntry]) async throws -> String {
