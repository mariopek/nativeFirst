...
BetAway has a data layer, a UI, and a persistence strategy. Users can log recovery check-ins and see their entries on a timeline. It works — but it is not smart yet. This is the lesson where BetAway gets a brain.

Most AI-powered apps reach for a cloud API at this point. That is a valid approach for many products, but BetAway takes a different path. As a gambling recovery app, privacy is non-negotiable — users are journaling about deeply personal struggles, financial damage, and emotional triggers. Sending that data to a cloud API is a design decision we reject on principle.

Instead, BetAway uses Apple's **NaturalLanguage** framework for on-device ML. All sentiment analysis runs locally on the device's Neural Engine. No API key. No network dependency. No data leaving the phone. Zero cost per analysis.

That said, understanding how cloud API integration works is a critical skill for any iOS developer. You will encounter projects that require it. So this lesson teaches both: we will walk through a production-quality OpenAI integration pattern — the networking layer, Keychain storage, retry logic, error handling — and then explain why BetAway deliberately chooses the on-device alternative. Learn the cloud pattern. Understand when to reject it.
...
- Understand why BetAway uses on-device ML instead, and when each approach is appropriate

## Why This Matters

Every AI-powered feature in a production app depends on one of two foundations: a secure, resilient networking layer for cloud AI, or an efficient on-device ML pipeline. If you hardcode your API key, it gets scraped from your binary within days. If you skip retry logic, users on flaky cellular connections see errors constantly. If your error types are stringly-typed, debugging production issues becomes guesswork.

The networking patterns in this lesson are not OpenAI-specific. They transfer to any authenticated API — Anthropic, Google Cloud, your own backend. Learn them once, use them everywhere. And then understand that for sensitive domains like recovery apps, the privacy-first approach of on-device ML is often the better choice.
...
- We want to understand how cloud sentiment analysis works, even
  though BetAway will use on-device ML (NaturalLanguage framework)
  for privacy reasons
...
Here is the complete, production-quality implementation. Every file compiles with Swift 6 strict concurrency enabled. Note: BetAway uses on-device ML for its actual sentiment analysis (covered in the next lesson). This OpenAI integration serves as a reference pattern for cloud API work you will encounter in other projects.
...
### Why BetAway Uses On-Device ML Instead

Now that you understand how cloud API integration works, here is why BetAway deliberately avoids it:

1. **Privacy** — Recovery journal entries contain deeply sensitive information: gambling amounts, emotional breakdowns, relationship damage, financial details. Sending this to any cloud API — even one with good privacy policies — is a trust violation. BetAway promises that journal text never leaves the device.

2. **Offline-first** — Users in crisis need the app to work immediately, regardless of network conditions. A cloud dependency means the app fails exactly when the user needs it most.

3. **Zero cost** — No API key management, no billing, no usage caps. The app is completely free and stays that way.

4. **Speed** — On-device sentiment analysis with NLTagger runs in under 10 milliseconds. A cloud round-trip takes 500ms to 2 seconds. For a crisis intervention feature, that latency matters.

The next lesson builds BetAway's actual sentiment analysis using Apple's NaturalLanguage framework — the approach we ship in production.
...
- [ ] You understand why BetAway uses on-device ML instead of this cloud approach
