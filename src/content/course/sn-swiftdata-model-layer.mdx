...
In this lesson, we design four SwiftData models, a rich set of domain enums, wire relationships with cascade delete rules, and learn the raw-value storage pattern that makes SwiftData and Swift enums play nicely together. We do it using the Plan-Execute-Verify workflow — the same workflow you will use for every feature in this course.
...
Every production iOS app needs a persistence layer, and SwiftData is Apple's answer for Swift-native apps. But SwiftData is opinionated — get the relationships, delete rules, or enum storage wrong and you hit runtime crashes that the compiler will not catch. The domain model layer we build here is the heart of BetAway. Every view, every chart, every notification depends on these types being correct.
...
1. UserProfile — the central model. Properties: id (UUID),
   ...
   DailyCheckin, UrgeLog, JournalEntry — all with cascade
   delete rules.

2. DailyCheckin — daily mood check-in. Properties: id, date,
...
This prompt is specific about the data architecture. It names every model with exact property types. It specifies relationship directions ("one-to-many from UserProfile") and delete rules ("cascade"). It explicitly asks for the raw-value storage pattern — because SwiftData cannot persist custom enums directly, you must store the `rawValue` String and expose a computed property. If you do not mention this, the AI will try to store enums directly and you will get runtime crashes.

It also calls out the JSON Data pattern for `EmergencyContact`. This is a real production technique — when you have a small, structured type that does not need its own table, encode it as JSON and store it as `Data` on the parent model. Simpler schema, fewer migrations.
...
This will produce code. It will also produce models that try to store Swift enums directly in SwiftData (crash), missing inverse relationships, `@Relationship` without delete rules (defaults to `.nullify` when you wanted `.cascade`), and no computed properties. Vague input produces vague output — always.
...
### Enums.swift — Domain Types

Before building any `@Model` classes, we define the enums. These drive the entire app — from the onboarding flow (picking gambling types) to the dashboard (displaying mood emoji) to the urge flow (selecting triggers and coping strategies).
...
- **Each enum owns its SF Symbol icon.** Instead of a mapping dictionary somewhere, the icon lives on the type. When you add a new gambling type, you are forced to add its icon — the compiler enforces it via the `switch` statement.
...
### UserProfile.swift — The Central Model
...
- **`@Relationship(deleteRule: .cascade)`** on all three arrays. When a `UserProfile` is deleted, all its checkins, urge logs, and journal entries are deleted too. This is correct — those records have no meaning without their parent profile.
- **`currentStreak` is a computed property**, not a stored value. It checks for the most recent relapse (an evening check-in where `gamblingFree == false`) and counts days from there. If there are no relapses, it counts from `quitDate`. This logic runs on every access, which is fine — the data set is small (one check-in per day).
...
- **`@Relationship(inverse: \UserProfile.checkins)`** declares the inverse side. Only one side declares the inverse — the other side (`UserProfile.checkins`) uses `@Relationship(deleteRule: .cascade)` without `inverse:`.
- **`gamblingFree: Bool`** — this is the critical field. A `false` value in an evening check-in resets the streak counter.
...
- **`trigger` and `copingStrategy` are optional** — the user might log an urge without identifying the trigger. The computed property returns `nil` when the raw string is empty or unrecognized.
...
The simplest model. `promptUsed` tracks which journal prompt the user was responding to — useful for analytics later.
...
This is **not** an `@Model` — it is a plain `Codable` struct stored as JSON `Data` inside `UserProfile`. Notice `callURL` strips non-numeric characters from the phone number and builds a `tel://` URL. This handles messy user input like "(555) 123-4567" gracefully.

### AI Output Review

Here is how I evaluate what the AI generated:

- **Raw-value storage pattern used consistently** — every enum property stores the raw value and exposes a computed getter/setter. This is the only safe way to use custom enums with SwiftData
- **Cascade delete rules on the parent side** — correct. Deleting a UserProfile cascades to all child records
- **Inverse declarations only on the child side** — correct. `DailyCheckin`, `UrgeLog`, and `JournalEntry` each declare `inverse:`, while `UserProfile` does not
- **No force unwraps** — every property has a default value or is genuinely optional
- **Computed properties for business logic** — `currentStreak` and `totalSaved` derive from the data, not stored redundantly
...
```
Add a Milestone struct (not @Model — it is static data) with
these properties: id (Int), days (Int), titleKey (String),
descriptionKey (String), icon (String — SF Symbol).

Include a static `all` array with milestones at: 1, 3, 7, 14,
30, 60, 90, 180, 365 days.

Add two static methods:
- current(for days: Int) -> Milestone? — returns the most
  recent milestone the user has passed
- next(for days: Int) -> Milestone? — returns the next
  upcoming milestone

Do not make this an @Model. It is a fixed data set — no
...
This is a good example of knowing when **not** to use SwiftData. Milestones are fixed data — they never change at runtime. Making them an `@Model` would add database overhead for no benefit. A static array is the simplest, fastest solution.
...
2. **Check the relationship inverses.** Open `DailyCheckin.swift` and confirm `@Relationship(inverse: \UserProfile.checkins)`. Do the same for `UrgeLog` and `JournalEntry`. Confirm `UserProfile` does NOT also declare `inverse:` on its relationship arrays — only one side declares the inverse.

3. **Check delete rules.** `UserProfile` should use `.cascade` on all three relationship arrays. This is correct for one-to-many where the parent owns the children. If the AI generated `.nullify`, change it — orphaned check-ins with no parent profile are useless data.

4. **Test the raw-value pattern.** Create a `DailyCheckin` with `checkinType: .morningPledge`. Print `checkinTypeRaw` — it should be `"Morning Pledge"`. Set `checkinType = .eveningCheckin`. Print `checkinTypeRaw` again — it should be `"Evening Check-in"`.
...
6. **Test persistence.** Insert a `UserProfile` and a `DailyCheckin`, quit the app, relaunch. Both should persist. Delete the profile — confirm the check-in is also deleted (cascade).
...
- [ ] Child models declare `inverse:` — parent model does not
...
Next up: **Lesson 2.1 — Design System & Theme**, where we build the `ColorPalette`, `Typography`, `Spacing`, and `AnimationPresets` that give BetAway its visual identity.
