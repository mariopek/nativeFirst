...
description: "Build a comprehensive test suite for BetAway using the Swift Testing framework and AI-generated tests, covering mock services, the makeSUT pattern, and async ViewModel testing that catches real bugs."
...
You have a working app. The check-in flow saves data, the recovery dashboard displays it, the on-device ML generates insights. Everything works when you tap through it manually. But you have no idea if it still works after the next change you make.
...
1. **Build a `MockRecoveryService`** with call tracking that verifies your ViewModels interact with the data layer correctly, without touching SwiftData.
...
Most indie developers skip tests because "the app is small enough to test by hand." That works until it does not. The first time you refactor the service layer and accidentally break the check-in save — but do not notice for three days because you were working on the streak tracking screen — you will wish you had a test that ran in two seconds and caught the regression immediately.
...
The app has:
- RecoveryDashboardViewModel: loads check-ins from RecoveryService,
  deletes check-ins, handles errors, manages loading state
- CheckInViewModel: multi-step form state, validation,
  saves via RecoveryService
- RecoveryService protocol with SwiftDataRecoveryService
  (production) and MockRecoveryService (tests)
- InsightEngine protocol for generating recovery insights
...
- ✅ **Test behavior, not implementation.** A good test says "when I call `loadCheckIns()`, the check-ins array is populated." A bad test says "when I call `loadCheckIns()`, `fetchAll()` is called on the service." The first survives refactoring. The second breaks every time you change the internal wiring.
- ✅ **Mock at the protocol boundary.** We already have `RecoveryService` as a protocol. The mock implements that protocol with in-memory storage and call tracking. No SwiftData, no disk, no flakiness.
- ⚠️ **Watch for over-mocking.** If the AI suggests mocking `Date`, `UUID`, and every system type, push back. Mock the things you own (services, insight engines). Use real value types for everything else.
...
@Test func loadCheckIns_callsServiceFetchAll() async {
    let mock = MockRecoveryService()
    let sut = RecoveryDashboardViewModel(service: mock)

    await sut.loadCheckIns()

    #expect(mock.fetchAllCallCount == 1)  // Brittle!
}
```

This test breaks the moment you add caching, debouncing, or batch loading — even though the behavior (check-ins appear) is unchanged. It tests how the ViewModel works, not what it does. A better test checks the observable outcome:

```swift
// GOOD — tests observable behavior
@Test func loadCheckIns_populatesCheckInsArray() async {
    let mock = MockRecoveryService()
    mock.checkIns = [CheckIn.sample()]
    let sut = RecoveryDashboardViewModel(service: mock)

    await sut.loadCheckIns()

    #expect(sut.checkIns.count == 1)
}
```
...
1. BetAwayTests/Mocks/MockRecoveryService.swift
   - Implements RecoveryService protocol
   - Stores check-ins in an in-memory array
...
2. BetAwayTests/Helpers/CheckIn+Sample.swift
   - Extension on CheckIn with static factory methods:
     .sample() returns a single check-in with sensible defaults
     .samples(count:) returns an array of check-ins

3. BetAwayTests/RecoveryServiceTests.swift
   - Tests for the MockRecoveryService itself — verify it
     stores, retrieves, updates, and deletes correctly
   - Use Swift Testing framework: @Test, #expect, @Suite

4. BetAwayTests/DashboardViewModelTests.swift
   - Tests for RecoveryDashboardViewModel
   - Use the makeSUT() pattern for setup
   - Test: loadCheckIns populates array, loadCheckIns sets
     loading state, loadCheckIns handles errors, deleteCheckIn
     removes from array, deleteCheckIn handles errors
...
final class MockRecoveryService: RecoveryService, @unchecked Sendable {
    ...
    private(set) var lastSavedEntry: CheckIn?
    private(set) var lastDeletedEntry: CheckIn?
    ...
    func fetchAll() async throws -> [CheckIn] {
        ...
        return checkIns.sorted { $0.date > $1.date }
    }
    ...
    func save(_ entry: CheckIn) async throws {
        saveCallCount += 1
        if let error = throwingError { throw error }
    ...
    func update(_ entry: CheckIn) async throws {
        updateCallCount += 1
        if let error = throwingError { throw error }
        guard let index = checkIns.firstIndex(where: { $0.id == entry.id }) else {
            return
        }
        checkIns[index] = entry
    }

    func delete(_ entry: CheckIn) async throws {
        deleteCallCount += 1
        if let error = throwingError { throw error }
        lastDeletedEntry = entry
        checkIns.removeAll { $0.id == entry.id }
    }
...
extension CheckIn {
    ...
    static func samples(count: Int) -> [CheckIn] {
        let calendar = Calendar.current
        let statuses: [RecoveryStatus] = [.great, .good, .okay, .bad, .awful]

        return (0..<count).map { index in
            CheckIn.sample(
                recoveryStatus: statuses[index % statuses.count],
...
- ✅ **Default parameters on `sample()`** — you can call `.sample()` for a generic entry or `.sample(recoveryStatus: .awful, note: "bad day")` when specific values matter for the test.
...
- ✅ **Cycles through recovery statuses** — gives variety in test data without randomness.

**RecoveryServiceTests.swift — testing the mock itself:**

```swift
import Testing
@testable import BetAway

@Suite("MockRecoveryService")
struct RecoveryServiceTests {

    // MARK: - makeSUT

    private func makeSUT() -> MockRecoveryService {
        MockRecoveryService()
    }
    ...
    func fetchAllReturnsSorted() async throws {
        let sut = makeSUT()
        let older = CheckIn.sample(
            date: Date(timeIntervalSinceNow: -86400)
        )
        let newer = CheckIn.sample(
            date: Date(timeIntervalSinceNow: 0)
        )
        sut.checkIns = [older, newer]
    ...
    func fetchByIDReturnsMatch() async throws {
        let sut = makeSUT()
        let target = CheckIn.sample()
        sut.checkIns = [CheckIn.sample(), target, CheckIn.sample()]
    ...
    func saveAppendsEntry() async throws {
        let sut = makeSUT()
        let entry = CheckIn.sample()

        try await sut.save(entry)

        #expect(sut.checkIns.count == 1)
        #expect(sut.checkIns.first?.id == entry.id)
    ...
    func updateReplacesEntry() async throws {
        let sut = makeSUT()
        let original = CheckIn.sample(note: "Original")
        sut.checkIns = [original]

        let updated = CheckIn.sample(
            id: original.id,
            note: "Updated"
        )
        ...
        #expect(sut.checkIns.count == 1)
        #expect(sut.checkIns.first?.note == "Updated")
    }
...
The service tests verify the infrastructure. Now we test the actual business logic. This is where the real value is.

```
Now write DashboardViewModelTests.swift using the makeSUT pattern.

RecoveryDashboardViewModel has:
- entries: [CheckIn] (populated by loadCheckIns)
- isLoading: Bool
- errorMessage: String?
- loadCheckIns() async
- addCheckIn(_ entry: CheckIn) async
- deleteCheckIn(_ entry: CheckIn) async

It takes a RecoveryService in its init.

Write tests for:
1. loadCheckIns populates the checkIns array
2. loadCheckIns sets isLoading to true during load
3. loadCheckIns sets errorMessage on failure
4. loadCheckIns clears previous error on success
5. addCheckIn inserts entry at the beginning
6. addCheckIn sets errorMessage on failure
7. deleteCheckIn removes the entry from the array
8. deleteCheckIn sets errorMessage on failure
9. entries are sorted by date descending after load

Use @Suite, @Test, #expect. Use makeSUT() that returns
(sut: RecoveryDashboardViewModel, service: MockRecoveryService).
...
@Suite("RecoveryDashboardViewModel")
@MainActor
struct DashboardViewModelTests {
    ...
    private func makeSUT() -> (
        sut: RecoveryDashboardViewModel,
        service: MockRecoveryService
    ) {
        let service = MockRecoveryService()
        let sut = RecoveryDashboardViewModel(service: service)
        return (sut, service)
    }

    // MARK: - Load Entries

    @Test("loadCheckIns populates entries from service")
    func loadCheckInsPopulatesArray() async {
        let (sut, service) = makeSUT()
        service.checkIns = CheckIn.samples(count: 3)

        await sut.loadCheckIns()

        #expect(sut.checkIns.count == 3)
    }

    @Test("loadCheckIns sets isLoading during fetch")
    func loadCheckInsSetsLoadingState() async {
        let (sut, service) = makeSUT()
        service.artificialDelay = .milliseconds(50)
        service.checkIns = [CheckIn.sample()]
    ...
    @Test("loadCheckIns sets errorMessage on failure")
    func loadCheckInsSetsErrorOnFailure() async {
        let (sut, service) = makeSUT()
        service.throwingError = .persistence(
    ...
    @Test("loadCheckIns clears previous error on success")
    func loadCheckInsClearsPreviousError() async {
        let (sut, service) = makeSUT()
    ...
    @Test("loadCheckIns returns entries sorted by date descending")
    func loadCheckInsReturnsSorted() async {
        let (sut, service) = makeSUT()
        let oldEntry = CheckIn.sample(
            date: Date(timeIntervalSinceNow: -86400)
        )
        let newEntry = CheckIn.sample(
            date: Date(timeIntervalSinceNow: 0)
        )
        service.checkIns = [oldEntry, newEntry]

        await sut.loadCheckIns()

        #expect(sut.checkIns.first?.id == newEntry.id)
        #expect(sut.checkIns.last?.id == oldEntry.id)
    }

    // MARK: - Add Entry

    @Test("addCheckIn inserts entry at the beginning of the array")
    func addCheckInInsertsAtBeginning() async {
        let (sut, service) = makeSUT()
        let existing = CheckIn.sample(
            note: "Existing",
            date: Date(timeIntervalSinceNow: -3600)
        )
        service.checkIns = [existing]
        await sut.loadCheckIns()
        ...
        await sut.addCheckIn(newEntry)

        #expect(sut.checkIns.first?.note == "New")
        #expect(sut.checkIns.count == 2)
    }

    @Test("addCheckIn sets errorMessage on failure")
    func addCheckInSetsErrorOnFailure() async {
        let (sut, service) = makeSUT()
        service.throwingError = .persistence(
    ...
    @Test("deleteCheckIn removes entry from the array")
    func deleteCheckInRemovesFromArray() async {
        let (sut, service) = makeSUT()
        let entry = CheckIn.sample()
        service.checkIns = [entry]
        await sut.loadCheckIns()
        #expect(sut.checkIns.count == 1)

        await sut.deleteCheckIn(entry)

        #expect(sut.checkIns.isEmpty)
    }

    @Test("deleteCheckIn sets errorMessage on failure")
    func deleteCheckInSetsErrorOnFailure() async {
        let (sut, service) = makeSUT()
        let entry = CheckIn.sample()
        service.checkIns = [entry]
        await sut.loadCheckIns()
...
- ✅ **`@MainActor` on the suite** — `RecoveryDashboardViewModel` is `@MainActor` because it drives UI. All tests that touch it must run on the main actor. Without this annotation, Swift 6 will reject the tests at compile time.
- ✅ **`makeSUT()` returns a tuple** — the test gets both the ViewModel and the mock service. This is critical: the test configures the mock, calls methods on the ViewModel, and asserts on the ViewModel's observable state. The tuple makes both accessible without instance variables.
...
- ✅ **Delete failure test verifies the entry is still present** — if the service throws during delete, the ViewModel should not remove the entry from its local array. This is a real bug that AI-generated code often has.
...
**5. Edge case: empty repository.** Add a test that calls `loadCheckIns()` on an empty service and verifies `entries` is empty and `errorMessage` is `nil`. This sounds trivial but catches ViewModels that set an error state when there are no results.

```swift
@Test("loadCheckIns with empty service sets empty array without error")
func loadCheckInsEmptyRepository() async {
    let (sut, _) = makeSUT()
...
### BetAwayTests/Mocks/MockRecoveryService.swift
...
### BetAwayTests/Helpers/CheckIn+Sample.swift
...
### BetAwayTests/RecoveryServiceTests.swift
...
struct RecoveryServiceTests {
    ...
    func deleteOnlyRemovesTarget() async throws {
        let sut = makeSUT()
        let keep = CheckIn.sample(note: "Keep")
        let remove = CheckIn.sample(note: "Remove")
        sut.checkIns = [keep, remove]

        try await sut.delete(remove)

        #expect(sut.checkIns.count == 1)
        #expect(sut.checkIns.first?.id == keep.id)
    }
    ...
    func resetClearsState() async throws {
        let sut = makeSUT()
        sut.checkIns = CheckIn.samples(count: 3)
...
struct DashboardViewModelTests {
    ...
    @Test("addCheckIn does not add to array on failure")
    func addCheckInDoesNotAddOnFailure() async {
        let (sut, service) = makeSUT()
        await sut.loadCheckIns()
        #expect(sut.checkIns.isEmpty)
    ...
    @Test("deleteCheckIn keeps entry in array on failure")
    func deleteCheckInKeepsEntryOnFailure() async {
        let (sut, service) = makeSUT()
        let entry = CheckIn.sample()
        service.checkIns = [entry]
        await sut.loadCheckIns()
    ...
    @Test("deleteCheckIn only removes the targeted entry")
    func deleteCheckInOnlyRemovesTarget() async {
        let (sut, service) = makeSUT()
        let keep = CheckIn.sample(note: "Keep")
        let remove = CheckIn.sample(note: "Remove")
        service.checkIns = [keep, remove]
        await sut.loadCheckIns()

        await sut.deleteCheckIn(remove)

        #expect(sut.checkIns.count == 1)
        #expect(sut.checkIns.first?.note == "Keep")
    }
}
```
...
- [ ] `MockRecoveryService` conforms to `RecoveryService` with no compiler warnings
- [ ] `CheckIn.sample()` creates valid entries with sensible defaults
- [ ] All `RecoveryServiceTests` pass — fetch, save, update, delete, error simulation
- [ ] All `DashboardViewModelTests` pass — load, add, delete, error states, sort order
...
**Test the `CheckInViewModel` form validation logic.**

Write a new test suite `CheckInViewModelTests` that covers:
1. `canAdvance` returns `true` on step 1 (status selection does not require validation)
...
8. `save(using:)` creates a `CheckIn` with the correct values from the form state
9. `reset()` returns all state to defaults

<details>
<summary>Hint</summary>

The `CheckInViewModel` does not depend on a service for most of its logic — only `save()` needs one. Your `makeSUT()` for this suite should still create a `MockRecoveryService`, but most tests will only manipulate the ViewModel's properties directly:
...
For the `save()` test, set all form properties, call `save(using: service)`, and then check `service.lastSavedEntry` to verify the entry was created with the correct recovery status, emoji, note, and tags.
</details>
