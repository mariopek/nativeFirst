...
Why? Because if you wait until halfway through development — or worse, until the app is "done" — you will face a massive refactoring job. Every hardcoded `"Sports Betting"` string, every `Text("Good Morning")`, every `static let disclaimer = "..."` becomes a search-and-replace nightmare. You have to audit every view, every view model, every enum, and every config. In BetAway, there are over 400 user-facing strings across 30+ files. Retrofitting localization onto a codebase that size is weeks of tedious, error-prone work.

But if you set up the pattern on day one, adding a new language later is purely a translation task. You hand the `Localizable.xcstrings` file to a translator, they fill in the strings, and the app works in that language. No code changes. No refactoring. BetAway went from 1 to 12 languages by adding `.lproj` bundles — the engineering work was already done because the localization system was built into the foundation.

The rule is simple: **if there is any chance your app will be multilingual, build the localization layer first.** The cost of doing it early is near zero. The cost of doing it late is proportional to the size of your codebase.
