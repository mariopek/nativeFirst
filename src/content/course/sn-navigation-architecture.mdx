...
description: "Implement a type-safe Router pattern with NavigationStack, enum-based routes, tab navigation, and sheet presentation for the mood entry flow."
...
topics: ["SwiftUI", "NavigationStack", "Router Pattern", "TabView", "Sheet", "Type Safety", "Navigation", "Moodbit"]
author: "Mario"
draft: false
pubDate: 2026-02-22
---
...
In this lesson we build the full navigation architecture for Moodbit: a central router, a type-safe route enum, tab navigation, and sheet presentation for the mood entry flow. We are going to do it the right way from the start so that deep linking, programmatic navigation, and future screens all slot in without refactoring.
...
- How to present `MoodEntryView` as a sheet from a floating action button
...
A centralized router solves this permanently. Every destination is an enum case. Every navigation action goes through one object. When you later add push notifications that open a specific mood detail, or a widget that jumps to the insights tab, the plumbing is already there. You parse the intent into a `Route` and hand it to the router. Done.
...
Before we touch code, let us get AI to think through the navigation architecture. Open Claude Code in the Moodbit project and send this:

```
I need to build the navigation architecture for Moodbit, a mood
tracking app. Before writing any code, think through the design.

The app has these screens:
- Timeline (main tab) — shows a list of mood entries
- MoodDetail — pushed from Timeline, shows a single MoodEntry
- Insights (second tab) — charts and statistics
- Settings (third tab) — user preferences
- MoodEntryView — presented as a sheet (modal) for logging a new mood

Requirements:
1. A Route enum that is Hashable, with cases for each destination.
   MoodDetail carries a MoodEntry as associated data.
...
4. The MoodEntryView is presented as a sheet triggered by a FAB
   (floating action button) on the Timeline tab.
...
Claude Code will outline the architecture — the Route enum with its cases, the AppRouter with `NavigationPath` and a `showMoodEntry` boolean, the MainTabView structure with three tabs, and a note about deep linking via `.onOpenURL`.

Read the plan. Here is what you should verify:

**The Route enum is flat, not nested.** You want `.moodDetail(MoodEntry)`, not `.timeline(.detail(MoodEntry))`. Flat enums are simpler and work directly with `NavigationPath`.

**Sheet state lives in the router.** The `showMoodEntry` boolean should be on `AppRouter`, not on the individual tab view. This way any part of the app can trigger the sheet — a button, a notification handler, a deep link.
...
```
Build the navigation architecture for Moodbit. Create three files:

1. Navigation/Route.swift — A Route enum conforming to Hashable
   with cases:
   - .timeline
   - .moodDetail(MoodEntry)
   - .insights
   - .settings
   MoodEntry must conform to Hashable for this to work.

2. Navigation/AppRouter.swift — An @Observable class with:
   - var path = NavigationPath()
   - var selectedTab: Tab = .timeline (Tab is a nested enum with
     cases: timeline, insights, settings — Int raw value)
   - var showMoodEntry = false
   - func navigate(to route: Route)
   - func navigateToRoot()
   - func presentMoodEntry()
   - func dismissMoodEntry()

3. Views/MainTabView.swift — A TabView with three tabs:
   - Timeline tab with a NavigationStack bound to router.path
   ...
   - The Timeline tab has a toolbar button (plus icon) that calls
     router.presentMoodEntry()
   - A .sheet modifier on the root view bound to
     router.showMoodEntry presenting MoodEntryView
...
enum Route: Hashable {
    case timeline
    case moodDetail(MoodEntry)
    case insights
    case settings
}
```

Four lines. That is all a route enum needs to be. Each case is a destination. The `.moodDetail` case carries its associated data — the `MoodEntry` the detail view needs to display. Because `Route` conforms to `Hashable` and `MoodEntry` conforms to `Hashable`, the whole thing works with `NavigationPath` out of the box.

Review checklist:

- ✅ Conforms to `Hashable` — required for `NavigationPath`
- ✅ Associated data on `.moodDetail` — the detail view gets its data from the route, not from a separate binding
- ✅ No associated data on `.timeline`, `.insights`, `.settings` — these are root views that do not need parameters
- ⚠️ Make sure your `MoodEntry` model conforms to `Hashable`. If it is a SwiftData `@Model`, it already conforms to `Identifiable`, but you may need to add `Hashable` explicitly. For a struct model, synthesized `Hashable` works automatically if all properties are hashable.
...
**`@Observable` instead of `ObservableObject`.** This is the modern approach. No `@Published` wrappers needed — every stored property is automatically observed. Any view that reads `router.path` or `router.showMoodEntry` will re-render when those values change.
...
**Explicit methods instead of direct property access.** You could let views set `router.showMoodEntry = true` directly. But methods like `presentMoodEntry()` are better because they name the intent, they are easier to find with search, and they give you a single place to add logic later — analytics, haptics, validation before presenting.

Review checklist:

- ✅ `@Observable` — modern observation
- ✅ `NavigationPath` — type-erased, Codable-ready
- ✅ Sheet state centralized — `showMoodEntry` lives here, not in a view
- ✅ Named methods — `navigate(to:)`, `presentMoodEntry()`, `navigateToRoot()`
- ⚠️ Check that `path` is only used by the Timeline tab's `NavigationStack`. If all three tabs share one path, navigation will break. We address this in `MainTabView`.
...
struct MainTabView: View {
    ...
    var body: some View {
        ...
        TabView(selection: $router.selectedTab) {
            Tab("Timeline", systemImage: "list.bullet", value: .timeline) {
        ...
        .sheet(isPresented: $router.showMoodEntry) {
            NavigationStack {
                MoodEntryView()
            }
        }
...
**Only the Timeline tab binds to `router.path`.** The Insights and Settings tabs have their own `NavigationStack` instances with no shared path binding. This means pushing a screen in the Timeline tab has zero effect on the other tabs. This is critical — shared navigation state across tabs is one of the most common SwiftUI bugs.

**The `.sheet` modifier on the `TabView`.** The sheet is attached at the root level, not inside a specific tab. This means it slides up over the entire tab bar, which is the correct iOS behavior for a modal. If you attach the sheet inside the Timeline tab's `NavigationStack`, it presents under the tab bar — visually broken.
...
- ✅ Only Timeline binds to `router.path` — independent navigation state
...
```
Update the App struct (MoodbitApp.swift) to create the AppRouter
...
like moodbit://mood/{id} for deep linking to a mood detail.
```
...
struct MoodbitApp: App {
    ...
    private func handleDeepLink(_ url: URL) {
        ...
        case "mood":
            ...
            // Fetch the MoodEntry by ID and navigate
            // router.navigate(to: .moodDetail(entry))
...
The deep linking handler is a stub right now — we will flesh it out when we have a data layer that can fetch a `MoodEntry` by ID. The important thing is the foundation is here. Parsing a URL into a tab selection and a route push is trivial when your navigation is state-driven.
...
action button (FAB) overlaid on the Timeline tab. It should be
...
The FAB floats above the timeline content using a `ZStack`. The `.bottomTrailing` alignment pins it to the bottom-right corner. The shadow gives it depth. This is a common mobile pattern — Material Design popularized it, and it works well in iOS apps for primary creation actions.
...
2. **Timeline navigation works.** If you have mood entries in your list, tap one. It should push to `MoodDetailView`. Tap the back button. You should return to the timeline.

3. **Tab state is independent.** Navigate deep into Timeline (push to a detail), switch to Settings, switch back to Timeline. You should be exactly where you left off — on the detail view, not the root.

4. **FAB presents the sheet.** Tap the floating plus button. `MoodEntryView` should slide up as a sheet over the tab bar. Dismiss it by swiping down.
...
- A `Route` enum with four cases, one carrying associated `MoodEntry` data
...
- A floating action button that presents `MoodEntryView` as a sheet
...
Moodbit will eventually need a "Quick Add" sheet — a simplified mood entry with just an emoji picker and a save button, no notes or tags. Implement this:

1. Add a `var showQuickAdd = false` property to `AppRouter` with `presentQuickAdd()` and `dismissQuickAdd()` methods.
2. Add a second `.sheet` modifier to `MainTabView` that presents a `QuickAddView` placeholder.
3. Add a long-press gesture to the FAB: a normal tap presents the full `MoodEntryView`, a long press presents `QuickAddView`.
