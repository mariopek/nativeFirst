...
description: "Build reliable UI test suites for BetAway using XCUITest, page object pattern, and AI-assisted test generation — covering DashboardView, MainTabView, the pledge flow, urge timer, and SettingsView."
...
XCUITest runs your actual app binary. It sees what the user sees. When you add a new feature and accidentally break the tab navigation in `MainTabView`, a UI test catches it in CI before your beta testers do. The cost is speed — UI tests take seconds, not milliseconds. The payoff is confidence that the screens your users touch actually work.
...
- **Reject tests that depend on network responses.** UI tests must run against mock data injected via launch arguments. If the AI suggests testing "loading indicators while fetching from CloudKit," that belongs in a unit test — not a UI test that depends on a server being available.
...
4. BetAwayUITests/BetAwayUITests.swift
   ...
   - Test cases:
     ...
     d. Urge flow — start timer, skip, log
...
```swift
// In BetFreeApp.swift — production code
@main
...
1. The tests are not isolated — if "morning pledge" fails
...
**Streak verification by value** — more precise than just checking existence:
...
```swift
// In DashboardView.swift — add identifiers to key elements
...
// In StreakCardView — add identifier to the streak count text
Text("\(profile.currentStreak)")
    .accessibilityIdentifier("dashboard-streak-count")

// In SavingsCardView — add identifier to the savings amount
...
The Journal page object follows the same pattern as the other page objects. Use `app.textFields` for the title field and `app.textViews` for the content body — XCUITest represents SwiftUI `TextField` and `TextEditor` differently:
