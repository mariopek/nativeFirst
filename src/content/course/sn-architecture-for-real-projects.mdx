...
3. Add a logger instance to AppRouter using
   os.Logger(subsystem: "com.nativefirst.betaway", category: "Router")
4. In BetAwayApp, wrap the ModelContainer creation in a static
...
├── Data/
│   └── (empty — services added in future lessons)
└── Tests/
...
- [ ] `BetAwayApp.swift` compiles with `ModelContainer` and router injection
...
2. Add a button in `HomeView` that navigates to `checkInDetail(id: UUID())` on tap
...
```swift
struct CheckInDetailPlaceholder: View {
    let checkInID: UUID

    var body: some View {
        Text("Check-In: \(checkInID.uuidString)")
...
Next up: **Lesson 1.2 — Data Modeling with SwiftData**, where we define the `CheckIn` model, set up relationships, and build the service protocol that the entire app depends on.
