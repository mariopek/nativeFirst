...
- How to version BetAway's `CheckIn` model from V1 to V2
...
This is especially dangerous with AI-assisted development. You ask Claude Code to "add a streakCount property to CheckIn" and it adds the property, compiles, runs great on the simulator — because the simulator has no existing data. You ship it. And then your users, who have weeks of recovery check-ins, open the app to a blank screen or a crash.
...
1. What does VersionedSchema look like for our current
   CheckIn model? Call it BetAwaySchemaV1.
2. For V2, I want to add two new properties:
   ...
   - sentiment: String (default "neutral") — stores the
     on-device ML-analyzed sentiment label
...
- **V1** captures the current CheckIn shape exactly as it exists in the database today
...
final class CheckIn {
    ...
    init(date: Date, recoveryStatus: Int, note: String) {
        self.date = date
...
1. BetAwaySchemaV1.swift — VersionedSchema with the current
   CheckIn model (date: Date, recoveryStatus: Int, note: String,
   tags: [String], isFavorite: Bool)
2. BetAwaySchemaV2.swift — VersionedSchema adding streakCount
...
conforming to VersionedSchema. The CheckIn inside each
...
- ✅ `models` returns an array of all model types in this schema version. If you had multiple models (CheckIn, UserProfile, etc.), they would all go here.
- ✅ The `CheckIn` class inside the enum is scoped to V1. This is the snapshot of what the model looked like at launch.
- ⚠️ Make sure this matches your actual shipped model exactly. If your production CheckIn has a property that is missing here, the migration will fail. Compare property-by-property.
    ...
    init() {
        do {
            modelContainer = try ModelContainer(
                for: BetAwaySchemaV2.CheckIn.self,
...
Notice the key change: `ModelContainer` now takes `BetAwaySchemaV2.CheckIn.self` — always the latest schema version — and `migrationPlan: BetAwayMigrationPlan.self`. This tells SwiftData: "The current model looks like V2. If the database is older, use this plan to bring it up to date."
...
Let me show you what that looks like. Suppose in V3 we want to replace the `recoveryStatus: Int` (a 1-5 scale) with a `recoveryLabel: String` ("crisis", "struggling", "managing", "steady", "strong"). This is not adding a column — it is transforming existing data. Lightweight migration cannot do this.

Here is the prompt:

```
Add a V3 to our migration plan. In V3, we replace the
recoveryStatus: Int property (1-5 scale) with recoveryLabel: String.

The custom migration should map:
1 -> "crisis", 2 -> "struggling", 3 -> "managing",
4 -> "steady", 5 -> "strong"
...
static let migrateV2toV3 = MigrationStage.custom(
    ...
    willMigrate: { context in
        guard let checkIns = try? context.fetch(
            FetchDescriptor<BetAwaySchemaV2.CheckIn>()
        ) else { return }

        for checkIn in checkIns {
            let label: String
            switch checkIn.recoveryStatus {
            case 1: label = "crisis"
            case 2: label = "struggling"
            case 3: label = "managing"
            case 4: label = "steady"
            case 5: label = "strong"
            default: label = "managing"
            }
            // The new property is set during migration
            checkIn.setValue(forKey: "recoveryLabel", to: label)
        }
...
@Test("V1 to V2 migration preserves existing check-ins")
func testV1toV2Migration() throws {
    ...
    let v1Container = try ModelContainer(
        for: BetAwaySchemaV1.CheckIn.self,
        configurations: v1Config
    )

    // Insert V1 data
    let context = v1Container.mainContext
    let checkIn = BetAwaySchemaV1.CheckIn(
        date: Date.now,
        recoveryStatus: 4,
        note: "Strong day in recovery",
        tags: ["exercise", "support group"],
        isFavorite: true
    )
    ...
    let v1CheckIns = try context.fetch(
        FetchDescriptor<BetAwaySchemaV1.CheckIn>()
    )
    #expect(v1CheckIns.count == 1)
    #expect(v1CheckIns[0].recoveryStatus == 4)
    #expect(v1CheckIns[0].note == "Strong day in recovery")
}
```
...
```swift
@Test("Migrated check-ins have correct default values")
func testMigratedDefaults() throws {
    ...
    for checkIn in checkIns {
        ...
        #expect(checkIn.streakCount == 0)
        #expect(checkIn.sentiment == "neutral")

        // Existing fields should be preserved
        #expect(!checkIn.note.isEmpty || checkIn.note == "")
        #expect(checkIn.recoveryStatus >= 1 && checkIn.recoveryStatus <= 5)
    }
}
```
...
- A check-in with an empty note and no tags
- A check-in with the maximum recovery status value (5)
- A check-in with a date far in the past
- A large number of check-ins (1,000+) to verify performance
...
1. **BetAwaySchemaV1 matches your shipped model exactly** — every property, every type. If there is a mismatch, the migration will fail on real user data.
2. **BetAwaySchemaV2 includes all V1 properties plus the new ones** — do not drop properties without handling it in a migration stage.
...
If all six are true, your data layer is production-safe. Your users can update the app without losing a single recovery check-in.

## Challenge

**Add a V3 migration (30 minutes):**

BetAway V3 needs a new property: `location: String?` — an optional string storing the city name where the check-in was logged. Since it is optional, no default value is needed. This is a lightweight migration.

1. Create `BetAwaySchemaV3.swift` with the updated model
2. Add a `migrateV2toV3` lightweight stage to `BetAwayMigrationPlan`
3. Update `schemas` and `stages` to include V3
4. Update `ModelContainer` to use `BetAwaySchemaV3.CheckIn.self`
5. Write a test that creates V2 data and verifies it migrates to V3 with `location == nil`

**Stretch goal:** Add a custom migration from V3 to V4 that splits the `tags: [String]` array into a separate `Tag` model with a relationship back to `CheckIn`. This requires a custom stage because you are moving data between models. Think through how `willMigrate` would iterate over check-ins, create `Tag` objects, and establish the relationships.
