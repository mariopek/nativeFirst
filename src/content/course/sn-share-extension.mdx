...
description: "Build a Share Extension for quick check-in logging from any app, use App Groups to bridge data between the extension and BetAway, and map out your next steps as an AI-augmented iOS developer."
...
- How to validate and extract shared content (text, URLs) from the extension context and attach it to a check-in
...
```
I want to add a Share Extension to BetAway so users can log a check-in
from any app's share sheet.

Current setup:
- SwiftData with CheckIn model (id, recoveryStatus, note, date, lastModified,
  aiInsight)
- App Groups already configured (group.com.nativefirst.betaway.shared) from the
  WidgetKit lesson
...
4. How do I write the check-in to the shared SwiftData store?
...
- ✅ **App Groups** — the extension target must have the same App Group identifier (`group.com.nativefirst.betaway.shared`) as the main app. The `SharedModelContainer` file must have target membership in both the main app and the extension.
- ⚠️ **Resource limits** — Share Extensions have approximately 120MB of memory and a few seconds to complete their work. If the plan does not mention these constraints, flag it. Long-running operations (like calling the OpenAI API for insights) must not happen in the extension.
- ⚠️ **Notifying the main app** — after the extension saves a check-in, it should call `WidgetCenter.shared.reloadAllTimelines()` to update widgets. There is no direct way to "poke" the main app, but SwiftData will pick up the new entry the next time the main app's `ModelContext` is accessed.
- ❌ If the plan suggests using `UserDefaults(suiteName:)` to pass recovery check-ins — reject. We have a shared SwiftData container. UserDefaults is for small preferences, not structured data.
...
5. Enabling the App Group (group.com.nativefirst.betaway.shared) on the new target
6. Adding CheckIn.swift and SharedModelContainer.swift to the
...
- ✅ Extension target created with correct bundle identifier (e.g., `com.yourname.betaway.share`)
- ✅ App Groups entitlement added to the extension target with `group.com.nativefirst.betaway.shared`
- ✅ `CheckIn.swift` and `SharedModelContainer.swift` have target membership in both the main app and the extension
- ✅ `NSExtensionActivationRule` configured to accept text and URLs with specific type rules
- ⚠️ If Info.plist still uses `TRUEPREDICATE` for the activation rule — this works for development but Apple will reject it during App Review. Replace it with specific type identifiers before submitting
- ❌ If the extension has its own `@Model` class separate from the main app's `CheckIn` — reject. Both targets must share the exact same model file. Two separate model definitions means two separate database schemas
...
2. ShareCheckInView.swift — the SwiftUI view shown in the share sheet.
   UI: a compact vertical layout with:
   - A row of 5 status buttons (emoji: awful through great)
...
3. SharedDataManager.swift — a helper that creates a CheckIn in
...
Requirements:
- Use the shared App Group container (group.com.nativefirst.betaway.shared)
...
- Keep the extension lightweight — no AI calls, no network requests
```
...
- ⚠️ If the view does not call `completeRequest` on cancel — the share sheet will hang. The user will have to force-quit the host app. Always complete the request on every exit path
- ❌ If the extension makes a network request to the OpenAI API — reject. Extensions have strict time limits. Defer AI analysis to the next time the main app opens
...
1. Add haptic feedback (UIImpactFeedbackGenerator) when the user
   taps a status button
2. Handle the edge case where SharedModelContainer.create() throws —
...
5. Animate the status button selection with a scale effect
...
3. Select a check-in, optionally edit the note, tap Save
4. Open the main BetAway app — does the new entry appear in the timeline?
5. Verify the check-in value, note text, and date are all correct
...
7. Log via extension while offline — does the entry save locally and sync when online?

**Widget update:**
1. After saving from the share sheet, check the home screen widget — does it update to reflect the new check-in?
...
final class ShareViewController: UIViewController {
    private let logger = Logger(
        subsystem: "com.nativefirst.betaway.share",
    ...
    private func presentShareUI(text: String?, url: URL?) {
        let shareCheckInView = ShareCheckInView(
            sharedText: text,
            sharedURL: url,
            onSave: { [weak self] recoveryStatus, note in
                self?.saveCheckIn(recoveryStatus: recoveryStatus, note: note)
            },
    ...
    private func saveCheckIn(recoveryStatus: Int, note: String) {
...
///
/// Displays a compact status selector and note field. The user picks
/// a check-in, optionally edits the note, and taps Save. The entire
...
struct ShareCheckInView: View {
    ...
    @State private var selectedStatus: Int = 3
    @State private var noteText: String = ""
    @State private var isSaved = false
    @State private var showError = false

    private let maxNoteLength = 500
    private let statusOptions: [(emoji: String, label: String, value: Int)] = [
        ("😣", "Awful", 1),
        ("😕", "Bad", 2),
        ("😐", "Okay", 3),
        ("🙂", "Good", 4),
    ...
    private func save() {
        ...
        onSave(selectedStatus, finalNote)
    }
}

// MARK: - Status Option Button

/// An individual status button with emoji and label.
///
...
struct StatusOptionButton: View {
    ...
    var body: some View {
        ...
        .buttonStyle(.plain)
        .accessibilityLabel("(label) recovery status")
...
/// Handles creating and saving recovery check-ins from any target
...
/// so callers can handle failure gracefully — especially important
/// in extensions where crashing produces a blank share sheet with
/// no error message.
enum SharedDataManager {
    private static let logger = Logger(
        subsystem: "com.nativefirst.betaway.shared",
        category: "DataManager"
    )

    /// Saves a check-in to the shared SwiftData store.
    ///
    /// - Parameters:
    ///   - recoveryStatus: The recovery status rating from 1 (awful) to 5 (great).
    ///   - note: Optional text note accompanying the check-in.
    ///   - date: When the check-in was logged. Defaults to now.
    ...
    static func saveCheckIn(
        ...
        do {
            ...
            logger.info("Check-in saved: recoveryStatus=\(recoveryStatus)")
    ...
    /// Fetches the most recent check-in for today.
    ///
    /// Used by both the main app and extensions to check whether
    /// a check-in has already been logged today.
    static func todaysCheckIn() -> CheckIn? {
        ...
        let predicate = #Predicate<CheckIn> { entry in
    ...
    /// Fetches the N most recent recovery status values.
    ///
    /// Useful for showing "recent statuses" as quick-select options
    /// in the Share Extension.
    static func recentStatusValues(limit: Int = 3) -> [Int] {
        guard let container = try? SharedModelContainer.create() else {
            return []
        }
        ...
        return entries.map(\.recoveryStatus)
    }
}
```
...
<dict>
    ...
    <array>
        <string>group.com.nativefirst.betaway.shared</string>
    </array>
    <key>com.apple.developer.icloud-container-identifiers</key>
    <array>
        <string>iCloud.com.yourname.betaway</string>
    </array>
...
**Why append the URL to the note instead of a separate property?** Adding a `sourceURL` property to `CheckIn` would require a SwiftData schema migration for existing users. For a v1 feature, appending "via domain.com" to the note text is pragmatic. If URL tracking becomes a core feature later, add the property with a proper migration in the next release.

## Checkpoint

1. Share Extension target created with bundle identifier `com.yourname.betaway.share`
2. App Group `group.com.nativefirst.betaway.shared` enabled on the extension target (matching the main app)
3. `CheckIn.swift` and `SharedModelContainer.swift` have target membership in both the main app and the extension
...
6. Selecting a check-in and tapping Save creates an entry visible in the main app
...
**Add a "Recent Statuses" quick picker to the Share Extension.**

When the share sheet opens, show the user's three most recent recovery status values as quick-select chips above the full status picker. If the user logged "Good" three times this week, they can tap the "Good" chip instead of scrolling through the emoji row. This requires reading from the shared SwiftData store in the extension.

<details>
<summary>Hint</summary>

`SharedDataManager` already has a `recentStatusValues(limit:)` method. Call it on appear and show unique values:

```swift
@State private var recentStatuses: [Int] = []

// In .onAppear, after pre-filling the note:
let recent = SharedDataManager.recentStatusValues(limit: 5)
recentStatuses = Array(Set(recent)).sorted()

// In the view body, above the full status row:
if !recentStatuses.isEmpty {
    HStack(spacing: 8) {
        Text("Recent:")
            .font(.caption)
            .foregroundStyle(.secondary)
        ForEach(recentStatuses, id: \.self) { status in
            let option = statusOptions.first { $0.value == status }
            Button(option?.emoji ?? "") {
                withAnimation(.spring(duration: 0.3)) {
...
Keep the fetch lightweight — `fetchLimit: 5` ensures you never load more than five entries into memory, which is important in the extension's constrained 120MB environment. The `Set` deduplication and `.sorted()` ensure you show at most 5 unique recovery statuses in order.
</details>
...
You have built a complete app. BetAway has a design system, a SwiftData persistence layer, Charts, AI insights, widgets, notifications, Siri Shortcuts, CloudKit sync, and now a Share Extension. That is not a tutorial project — that is a real product.
...
**Collect user feedback systematically.** Add a simple feedback mechanism in the app — a "Send Feedback" button in Settings that opens a pre-filled email. Track every piece of feedback in a spreadsheet or GitHub Issues. Patterns will emerge fast: if three out of five testers ask for the same thing, that is your next feature.

**Marketing starts before launch.** Write three tweets or posts showing off BetAway — the widget on a home screen, the AI insight for a check-in, the share sheet in action. Screenshots sell apps. Build an audience before you have something to sell them.
...
- **Recovery streaks** — gamify consistency. Show the user how many consecutive days they have logged a check-in. Streaks drive retention more effectively than features.
- **Export to PDF** — let users export a week or month of recovery data as a formatted PDF report. Use Apple's `UIGraphicsPDFRenderer` and the Charts framework data you already have.
- **Therapist sharing** — generate a summary report that users can share with their therapist. This is the feature that turns BetAway from a personal tool into a health tool.
- **StoreKit 2 tip jar** — BetAway is completely free, but a voluntary tip jar lets grateful users support development. StoreKit 2 makes this straightforward. Apple handles billing and refunds.
