...
BetAway supports English, German, French, Spanish, Italian, Portuguese (Portugal), Portuguese (Brazil), Russian, Japanese, Korean, Arabic, and Balkan (Croatian/Serbian/Bosnian) — 12 languages covering the majority of gambling markets worldwide. String Catalogs make adding each language a translation task, not an engineering task.

### Set Up Localization From Day One

If you know your app will support multiple languages — even if you plan to launch with just one — set up the localization infrastructure at the very beginning of the project. This means creating the `LanguageManager`, establishing the `localizedName` pattern for enums, and using `LanguageManager.shared.localized()` for every user-facing string from the first commit.

Why? Because if you wait until halfway through development — or worse, until the app is "done" — you will face a massive refactoring job. Every hardcoded `"Sports Betting"` string, every `Text("Good Morning")`, every `static let disclaimer = "..."` becomes a search-and-replace nightmare. You have to audit every view, every view model, every enum, and every config.
...
If you built the entire app in English first and then decided "let's add 11 more languages," the work breaks down into two phases — and the first one is the killer:

**Phase 1 — Refactoring (the engineering nightmare):** You have to find every hardcoded string in 30+ files, replace each one with `LanguageManager.shared.localized("...")`, convert every `static let` into a `computed var`, add `localizedName` to every enum, make sure no view uses `.rawValue` for display, and test that nothing broke. For BetAway's codebase, this is easily 2-3 full days of careful, file-by-file auditing. If you are using AI-assisted development, it is hundreds of thousands of tokens spent on refactoring — the AI has to read every file, understand the context, rewrite strings, and you have to review every change. It is the worst kind of AI task: high token cost, low creative value, and high risk of subtle bugs (a missed string that stays English, a format string with wrong `%@` placement).

**Phase 2 — Translation (the easy part):** Once the infrastructure is in place, you export the `Localizable.xcstrings` file and hand it to translators. Each language needs ~400 strings translated. This is a content task, not an engineering task. It scales linearly and can be done in parallel by different translators.

But if you set up the pattern on day one, **Phase 1 never exists.** You write `LanguageManager.shared.localized("Good Morning")` instead of `"Good Morning"` from the start — it takes the same amount of effort. Every new enum gets `localizedName` as part of its initial implementation. Every `AppConfig` property starts as a computed `var`. When it is time to add languages, you skip straight to Phase 2. No refactoring. No auditing. No wasted tokens.

BetAway went from 1 to 12 languages by adding `.lproj` bundles — the engineering work was already done because the localization system was built into the foundation.
...
switching. Do NOT write code yet — just audit and plan.
...
3. AppConfig computed properties: missionStatement, medicalDisclaimer,
   appTagline — all must use LanguageManager for runtime localization
...
**Keep** — The `LanguageManager` should be a singleton (`static var shared`) so it can be accessed from enums and services that are not SwiftUI views. Enums like `GamblingType` are plain Swift types — they cannot use `@Environment` — so they need `LanguageManager.shared.bundle` to look up translations.

**Keep** — Every enum should have a `localizedName` computed property that calls `LanguageManager.shared.bundle.localizedString(forKey:value:table:)` using the enum's `rawValue` as the key. This way the raw value is the English string (which doubles as the default) and the translation comes from the `.lproj` bundle.

**Question** — If the AI suggests using `String(localized:)` for everything, push back. `String(localized:)` uses the system locale, not the app's chosen language. BetAway's runtime language switching requires using the bundle from `LanguageManager` directly.

### Anti-Pattern: Using System Locale Instead of App Language

```swift
// Do NOT do this — uses system locale, ignores app language setting
Text(String(localized: "Good Morning"))

// Do NOT do this — rawValue is English, not localized
Text(gamblingType.rawValue)

// Do NOT do this — static string that cannot change at runtime
...
3. AppConfig uses computed properties (var, not let) for
   missionStatement, medicalDisclaimer, appTagline — all
...
final class LanguageManager {
    // MARK: - Singleton (for use outside SwiftUI environment — enums, services, etc.)
    ...
    // MARK: - State
    /// The currently active language — drives the whole app's localization.
    ...
    /// Locale for SwiftUI environment — makes Text("literal") auto-localize.
    var locale: Locale {
    ...
    init() {
        ...
        if let savedCode = UserDefaults.standard.string(forKey: Self.languageOverrideKey),
           let saved = AppLanguage(rawValue: savedCode) {
            ...
            // 2. No override — detect from device locale
    ...
    static func detectBestMatch() -> AppLanguage {
        ...
        for langCode in preferred {
            ...
            // Try matching just the language prefix (e.g. "de-AT" → "de")
    ...
    /// Reset to "System Default" — remove the override.
    func resetToSystem() {
...
- `detectBestMatch()` handles Balkan languages specially — Croatian (`hr`), Serbian (`sr`), and Bosnian (`bs`) all map to `.balkans` because the written forms are mutually intelligible.
...
- `localized(_:)` uses `bundle.localizedString(forKey:value:table:)` instead of `String(localized:)`. This is the key difference — `String(localized:)` uses the system locale, while `bundle.localizedString` uses whichever `.lproj` bundle the `LanguageManager` has resolved.
...
1. The `rawValue` is the English string — `"Sports Betting"`, `"Daily"`, `"Watching live sports"`, etc.
...
The `MoodRating` enum uses `LanguageManager.shared.localized()` instead of `bundle.localizedString()` directly — both work, but `localized()` is the convenience wrapper. The choice is cosmetic.

### Badge.BadgeCategory — Another localizedName Example
...
### AppConfig — Localized Computed Properties

`AppConfig` uses computed properties (`var`, not `let`) for any string that needs localization. The mission statement and medical disclaimer are the most important — they appear in Settings and contain the app name and helpline number, which are interpolated using format strings:
...
3. **`helplines(for:)` returns different phone numbers per language.** A German user sees the BZgA number. A Japanese user sees the Japanese gambling helpline. This is not just translation — it is content that changes entirely based on locale.
...
The `displayName` property shows the native name — "Deutsch" for German, "Francais" for French — so a user who does not read English can still find their language. The `flag` emoji provides visual identification.

### SettingsView — Language Section
...
```swift
// Inside SettingsView — Language Section
Section {
    Button {
...
### BetFreeApp — Wiring It All Together
...
```swift
// In MorningPledgeView — success toast
PledgeSuccessToast(
...
```swift
// In EveningCheckinView — success toast
PledgeSuccessToast(
...
```swift
// In EveningCheckinView — relapse helpline
...
1. The Settings view displays gambling types using .localizedName
   but the list is joined with ", " — for Arabic (RTL), the
...
```swift
// In SettingsView — Profile Section
HStack {
...
1. **Language switching** — Open Settings, change the language from English to Japanese. Every screen should update immediately: dashboard greeting, pledge reasons, mood labels, milestone descriptions, badge titles, settings labels. No English strings should remain except proper nouns like "BetAway."
2. **Enum localization** — In the onboarding gambling profile screen, check that all 9 `GamblingType` options display in the current language. Switch to Korean and verify they show Korean text.
3. **Medical disclaimer** — In Settings, scroll to the disclaimer. Switch languages. The disclaimer text should change but always include the correct helpline number for the current language.
4. **Helpline numbers** — Switch to German. The About section should show "BZgA Glucksspielsucht" with the German number. Switch to Japanese. It should show the Japanese gambling helpline number.
5. **Milestone descriptions** — Check the progress/milestones screen. All titles ("Day 1", "1 Week", "90 Days") and descriptions ("The hardest step is the first one") should be in the current language.
6. **Notification content** — With the app in French, toggle notifications off then on. The morning notification should say the French version of "Good Morning" / "Start your day with a pledge to stay bet-free."
7. **First launch detection** — Delete the app and reinstall. Set the device language to Spanish before launching. The app should auto-detect Spanish and start localized without the user needing to visit Settings.

## Final Code

The complete `LanguageManager`, `Enums.swift` with all `localizedName` properties, and `AppConfig` with localized computed properties are shown in the Execute Phase above. Here is how they connect in the Settings view where all three systems converge — language picker, localized enums, and localized config:
...
struct SettingsView: View {
    ...
    var body: some View {
        List {
            if let profile {
                // Profile Section — uses enum .localizedName
                Section {
                    HStack {
                ...
                // Language Section — LanguageManager integration
                Section {
                    Button {
                        showLanguagePicker = true
                    } label: {
                ...
                // About Section — language-aware helplines
                Section {
                ...
                // Disclaimer — localized computed property
                Section {
...
1. **Enum `.localizedName`** — `profile.gamblingTypes.map(\.localizedName)` translates each gambling type.
2. **`LanguageManager` environment** — `languageManager.currentLanguage` drives the language picker display and helpline lookup.
3. **`AppConfig` computed property** — `AppConfig.medicalDisclaimer` re-evaluates through `LanguageManager.shared.localized()` on every access.
