...
description: "Learn production-quality cloud API integration patterns — secure Keychain storage, async/await networking, typed errors, retry logic — and understand why BetAway deliberately rejects cloud AI in favor of privacy-first manual mood tracking."
...
And here is the `JournalEntry` model — the SwiftData model where mood is stored as a plain integer, selected by the user:

```swift
// BetFree/Models/JournalEntry.swift — what BetAway actually ships
...
Notice what is **not** here: no sentiment score, no confidence value, no AI-generated themes, no API call. The `mood` is an `Int` that the user selects through a mood picker. The `content` is stored locally via SwiftData and never sent anywhere. The `promptUsed` is one of several optional writing prompts (like "What am I grateful for?") — not an AI prompt, but a journaling prompt for the user.

Here is how the journal entry view collects mood — the user taps an emoji to select their state. This is from the real `JournalEntryView.swift`:
...
And here is how BetAway tracks mood over time without any AI — the `ProgressViewModel` computes averages directly from the user's self-reported mood integers:
...
**Check the Keychain approach.** If the plan suggests UserDefaults, Environment variables at runtime, or a hardcoded string, reject it immediately. BetAway's real codebase does not store any API keys because it does not call any cloud APIs — but if it did, the Keychain is the only acceptable location.
...
```swift
// Reference implementation — DO NOT DO THIS
...
1. **Privacy** — Recovery journal entries contain deeply sensitive information: gambling amounts, emotional breakdowns, relationship damage, financial details. Sending this to any cloud API — even one with good privacy policies — is a trust violation. BetAway's `JournalEntry.content` is stored locally in SwiftData and never leaves the device.

2. **User agency** — BetAway's `MoodRating` lets the user declare their own emotional state. An algorithm guessing "you seem negative" based on text analysis can be wrong, patronizing, or triggering. The user tapping a mood emoji is always accurate to their experience.

3. **Offline-first** — Users in crisis need the app to work immediately, regardless of network conditions. The `EveningCheckinView` mood selector works in airplane mode, underground, anywhere. A cloud dependency means the app fails exactly when the user needs it most.

4. **Zero cost** — No API key management, no billing, no usage caps. BetAway is completely free and stays that way. The real `AppConfig.swift` has no API-related configuration at all.

5. **Simplicity** — The `ProgressViewModel.moodData(from:days:)` method computes mood trends by averaging integers. No parsing, no error handling, no retry logic. Simple code has fewer bugs.

The real BetAway check-in flow — from the actual `DailyCheckin` model — stores mood as a raw integer alongside gambling-free status, triggers, and notes:

```swift
// BetFree/Models/DailyCheckin.swift — what BetAway actually ships
...
- [ ] You can identify the `APIError` pattern — `LocalizedError` conformance, typed throws, retryable classification
...
- [ ] Retry logic only retries on 429 and 5xx — not 401 or 400
...
- [ ] Searching BetAway's real codebase for `sk-`, `OpenAI`, `NLTagger`, or `APIKey` returns zero results — the real app has no AI dependencies
