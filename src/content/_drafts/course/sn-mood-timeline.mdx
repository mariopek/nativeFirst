...
description: "Build the main dashboard view — a card-based ScrollView with greeting header, StreakCardView, QuoteCardView, SavingsCardView, StatsCardView, DashboardPledgeCard, WeeklySummaryCard, MilestoneCardView, and EmergencyUrgeButton, backed by a DashboardViewModel using @Observable."
...
The data layer exists. Models are defined, the service layer is wired, SwiftData persists user profiles. Now we build the screen users will open dozens of times a day — the recovery dashboard.
...
3. Card components — `StreakCardView`, `QuoteCardView`, `SavingsCardView`, `StatsCardView`, `DashboardPledgeCard`, `WeeklySummaryCard`, `MilestoneCardView` — each encapsulating one piece of the recovery dashboard
...
Every health app, recovery app, and habit tracker has a dashboard. The patterns here — card-based composition, staggered animations for polish, `@Query` for reactive SwiftData reads, computed properties for time-based UI, and milestone progress tracking — show up in virtually every production iOS app. Master them once and you reuse them everywhere.
...
```
I need to build the home screen for BetFree — a rich card-based
...
- **DashboardViewModel.swift** — `@Observable` class with time-based greeting, milestone progress, and next milestone text
- **DashboardView.swift** — the main screen with `ScrollView`, `VStack`, `@Query` for `UserProfile`, staggered animations, and `.fullScreenCover` for urge flow
- **StreakCardView.swift** — gradient card with streak count, progress ring, and milestone labels
- **QuoteCardView.swift** — daily quote in a serif-styled card
- **SavingsCardView.swift** — money saved with annualized pace
- **StatsCardView.swift** — hours reclaimed display (often in the same file as SavingsCardView)
- **DashboardPledgeCard.swift** — morning pledge and evening check-in tracker
- **WeeklySummaryCard.swift** — weekly stats with `@Query` for urge logs, check-ins, and journal entries
- **MilestoneCardView.swift** — current/next milestone with progress bar
- **EmergencyUrgeButton.swift** — pulsing button in `QuickActionsView`
- **MainTabView.swift** — tab container with custom animated tab bar
...
- **A flat list of check-ins** — the real dashboard is not a timeline of log entries. It is a composition of purpose-built cards that each show a different facet of recovery. Building a simple `LazyVStack` of check-in rows misses the entire architecture.
- **All logic in the View** — time-based greetings, milestone math, and progress calculations belong in the ViewModel. Putting `Calendar.current.component(.hour, from: Date())` directly in the view body makes it untestable and cluttered.
- **A single monolithic file** — cramming greeting, streak, quote, savings, pledge, summary, milestones, and the emergency button into one `DashboardView.swift` produces a 500+ line file that nobody can maintain. Each card is its own file.
...
2. WeeklySummaryCard should use @Query with sort descriptors for
   ...
   card reactive — when a new urge is logged, the summary updates
...
1. **Greeting header** — open the app at different times. Morning shows "Good morning" with a sun icon, afternoon shows "Good afternoon" with a sun icon, evening shows "Good evening" with a moon, night shows "Good night" with a moon. The greeting line below says "Day X of your journey" matching the user's current streak.

2. **Streak card** — the gradient card displays the current bet-free streak with an animated counter. The progress ring on the right shows how far the user is toward the next milestone. Below the streak count, the next milestone countdown text reads something like "5 day(s) to First Week."

3. **Quote card** — a motivational quote renders in serif font with an author attribution. The quote changes daily based on `MotivationalQuotes.quoteForToday()`.

4. **Savings and stats row** — two side-by-side cards. The left card shows total money saved with an annualized pace below it. The right card shows hours reclaimed.

5. **Pledge card** — shows "Morning Pledge" and "Evening Check-in" with pending/completed status. If tasks remain, a CTA button appears ("Take Today's Pledge" or "Complete Evening Check-in"). When both are done, a green "Complete" label replaces the CTA.

6. **Weekly summary** — four stats in a row: pledges (X/7), urges, resistance percentage, journals. A motivational message below adapts to the user's actual performance.

7. **Milestones** — current milestone shows with a checkmark and progress bar toward the next. If no milestone is achieved yet, it shows the first target.

8. **Emergency urge button** — a pulsing button at the bottom with "Feeling an urge?" and "Tap here for immediate support." Tapping it opens the `UrgeContainerView` as a full-screen cover.

9. **Staggered animations** — on first appear, each card fades and slides in with a staggered delay. This gives the dashboard a polished, cascading reveal.

10. **No profile edge case** — if no `UserProfile` exists (should not happen after onboarding), the view shows a "Setting up your profile..." fallback instead of crashing.
...
**`milestoneProgress(for:)` takes the profile as a parameter.** The ViewModel does not own the profile — the view gets it from `@Query`. The ViewModel computes derived values from whatever profile the view passes in. This keeps the ViewModel lightweight and avoids duplicating the SwiftData source of truth.

**The progress calculation** finds the distance between the current milestone and the next, then computes how far the user's streak has progressed between them. If there is no next milestone (the user has achieved the highest one), it returns `1.0` — the ring is full.

**`nextMilestoneText(for:)` returns an optional.** When the user has achieved the highest milestone, there is no "next" — the text is `nil` and the view can hide the countdown label. The localized string uses `%lld` for the integer day count, matching Apple's localization format for plurals.
...
**`@Query private var profiles: [UserProfile]` with `profiles.first`.** SwiftData's `@Query` returns an array even when you expect one record. The computed property `profile` unwraps `profiles.first` so the rest of the view can use optional binding cleanly. When SwiftData updates the profile (streak changes, pledge completes), the `@Query` automatically triggers a view update — no manual refresh needed.
...
**`.fullScreenCover(isPresented: $showUrgeFlow)` for the urge flow.** The emergency urge button sets `showUrgeFlow = true`, which presents `UrgeContainerView` as a full-screen modal. This is deliberate — when a user is experiencing a gambling urge, the support flow should take over the entire screen. A sheet that can be dismissed with a swipe is too easy to abandon.
...
**No `.task` or `.refreshable`** — unlike a list-based dashboard, the card-based layout reads from `@Query` and computed properties. The data is reactive through SwiftData. There is no async fetch to trigger.
...
**`BFGradientCard` wraps the entire card.** This is a reusable design-system component that applies a gradient background with rounded corners and padding. The streak card uses `ColorPalette.primaryGradient` — the app's signature gradient — so it stands out as the most important card on the dashboard.
...
**`BFProgressRing` shows milestone progress.** The ring fills proportionally between the current milestone and the next. The `viewModel.milestoneProgress(for:)` call returns a 0-to-1 value. Inside the ring, the current milestone's icon is overlaid — a visual anchor for what the user has already achieved.
    ...
    var body: some View {
        BFCard {
            VStack(alignment: .leading, spacing: Spacing.sm) {
                ...
                // Quote text — serif font for literary feel
                Text(quote.text)
...
**`.fixedSize(horizontal: false, vertical: true)`** prevents the quote text from being truncated. Quotes vary in length — some are one line, others are three. This modifier tells SwiftUI to expand vertically as needed without constraining width.
...
**These two cards live in the same file** because they are structurally identical — icon + label header, animated counter, subtitle. They sit side-by-side in an `HStack` on the dashboard, forming a compact stats row.

**`AnimatedDecimalCounter` for both values.** Like `AnimatedCounter` for the streak, this component animates when the value changes. `decimals: 0` rounds to whole numbers — "$1,247" not "$1,247.50" — because precision at this level would be false accuracy for estimated savings.

**The annualized pace** extrapolates current savings to a yearly rate. This reframes the number psychologically — "$42 saved" feels small, but "$15,330/yr pace" feels significant. It is the same data, presented to motivate.
...
**The entire card is a `Button`.** Tapping anywhere on the card opens the pledge flow. When all daily tasks are complete, `.disabled(allDone)` prevents further taps and the CTA button disappears, replaced by a green "Complete" label. This is a clear state transition — incomplete has an action, complete does not.
...
struct WeeklySummaryCard: View {
    ...
    private var avgMoodEmoji: String {
        ...
        return MoodRating(rawValue: Int(avg.rounded()))?.emoji ?? "😐"
    }
...
**Three `@Query` properties, each with its own sort.** `WeeklySummaryCard` runs its own SwiftData queries for `UrgeLog`, `DailyCheckin`, and `JournalEntry`. This is the correct pattern — each view that needs data from SwiftData uses `@Query` directly, rather than passing arrays down from a parent. SwiftData keeps these reactive. When the user logs a new urge or writes a journal entry, the weekly summary updates automatically.

**Computed properties filter to the last 7 days.** The `@Query` fetches all records (sorted by date descending), and computed properties like `weekUrges` filter to those within the last week. This is a pragmatic tradeoff — SwiftData `@Query` does not support dynamic date predicates easily, so we fetch all and filter in memory. For a recovery app with at most a few hundred records total, this is fine.

**The motivational message adapts to actual behavior.** It is not random — it reads the user's real data and picks the most relevant encouragement. Six or more pledges? Celebrate commitment. All urges resisted? Celebrate willpower. This makes the dashboard feel personalized, not generic.
    ...
    var body: some View {
        VStack(spacing: Spacing.md) {
            ...
            if let current = currentMilestone {
                ...
                // No milestone yet — show first target
                BFCard {
                    HStack(spacing: Spacing.md) {
                        ZStack {
                            Circle()
...
**Two states: milestone achieved and no milestone yet.** The card branches on whether `Milestone.current(for:)` returns a value. If the user has hit at least one milestone, the card shows it with a checkmark and a progress bar toward the next. If not, it shows the first target as motivation. This eliminates any empty state — there is always something to display.
...
**The action is a closure, not a navigation link.** The parent (`DashboardView`) passes `{ showUrgeFlow = true }`, which triggers a `.fullScreenCover`. This separation means the button does not know how the urge flow is presented — it just calls the closure.
...
**The system tab bar is hidden.** `.toolbar(.hidden, for: .tabBar)` removes the default iOS tab bar entirely. In its place, a custom `CustomTabBar` sits in a `VStack` below the `TabView`. This gives full control over the tab bar design — icon sizes, animations, the active indicator capsule, and haptic feedback.
...
**`@Bindable var router = router`** inside the body. The `AppRouter` is an `@Observable` class injected via `@Environment`. To create bindings to its properties (`selectedTab`, `navigationPath`), we need `@Bindable`. This local binding pattern is an iOS 17 `@Observable` idiom — it replaces the old `@ObservedObject` binding behavior.
