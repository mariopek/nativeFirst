...
BetAway chose an even simpler path: let the user tell you how they feel. No model inference at all. But if your app needs automated text analysis — a content moderation system, a customer feedback classifier, a writing assistant — NLTagger is the privacy-preserving foundation to build on.
...
```swift
// BetFree/Models/Enums.swift — what BetAway actually ships
...
```swift
// BetFree/Views/Journal/JournalEntryView.swift — what BetAway actually ships
...
Notice the `JournalPrompt` enum that provides optional writing prompts — these are for the human, not for an AI:
...
The evening check-in follows the same pattern — manual mood selection, no text analysis. From the real `PledgeViewModel`:

```swift
// BetFree/ViewModels/PledgeViewModel.swift — what BetAway actually ships
...
```swift
// BetFree/ViewModels/ProgressViewModel.swift — what BetAway actually ships
...
The weekly summary card on the dashboard uses the same approach — average the mood integers, display an emoji:

```swift
// BetFree/Views/Dashboard/WeeklySummaryCard.swift — what BetAway actually ships
...
Now let us build what BetAway could have used instead: Apple's NaturalLanguage framework for automated sentiment analysis of journal text. This is a reference implementation — the real app does not ship it.
...
4. This is a reference implementation — BetAway actually uses
...
- **Language support** — NLTagger's sentiment model works best with English. For other languages, accuracy drops. BetAway's real `LanguageManager` supports 12 languages — NLTagger would not serve all of those users equally.
...
This is the "always require network for basic features" anti-pattern. BetAway avoids it entirely — not by using on-device ML, but by skipping automated analysis altogether. The user's manual mood selection never fails, never throws, never needs a network connection.
...
```swift
// Reference implementation — not shipped in BetAway
...
- **Score mapping** — The ranges map to the same 1-5 scale as BetAway's real `MoodRating` enum. The thresholds are evenly distributed across the -1 to 1 spectrum. You could adjust these later based on real-world usage.
...
- **Never throws** — Notice that even though the protocol method is `throws`, `RecoveryService` never actually throws. On-device analysis cannot fail in any meaningful way. If the text is empty, you get neutral. If the text is gibberish, you get neutral. The model always returns something.
- **Recovery-focused insights** — These are static strings tailored to gambling recovery. BetAway's real app uses a similar approach for motivational messages — the `WeeklySummaryCard` generates encouragement based on pledge counts and urge resist rates, not based on NLP.
...
Same pattern — template-based messages selected by condition — but BetAway's conditions are behavioral (pledges made, urges resisted, journals written) rather than NLP-derived. The insight comes from what the user **did**, not what an algorithm inferred from their text.
...
Compare this to BetAway's real ViewModel. The actual `PledgeViewModel` has no analysis step — the user selects their mood directly:
...
|---|---|---|
| **Accuracy** | Moderate — aggregate sentiment only | Perfect — user knows their own state |
| **Insight quality** | Template-based, derived from score | Template-based, derived from behavior |
| **Latency** | Under 10ms | Instant — no computation |
...
The NLTagger approach wins when you cannot ask the user — when you need to classify text automatically, in bulk, without human input.
...
```swift
// Reference tests — for the NLTagger implementation that BetAway does not ship
...
- How `NLTagger` with `.sentimentScore` works — assign text, call `tag(at:unit:scheme:)`, parse the raw value to a `Double`
- How to map NLTagger's -1.0 to 1.0 range onto a 1-5 recovery score that matches BetAway's `MoodRating` scale
- Why BetAway chose manual mood tracking over NLTagger — user agency, multilingual support, code simplicity
...
- Zero new dependencies, zero API costs, zero privacy concerns — both the NLTagger reference and BetAway's actual approach share these properties

The key takeaway: BetAway demonstrates that you do not always need ML to solve a problem. The simplest solution — asking the user — is often the most accurate, most reliable, and easiest to maintain. On-device ML is a powerful tool for situations where automated text classification is genuinely needed. Know both patterns so you can choose the right one.
